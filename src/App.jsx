...
// ── KEYS ─────────────────────────────────────────────────────────────────────
const K = {
  tabs:"pg_tabs", cargas:"pg_c", medidas:"pg_m", migrDom:"pg_migr_dom1",
...
const BASE_EXLIB_LIST=[
  ...
  {nombre:"Dominadas lastradas",grupo:"Espalda",tipo:"Peso corporal",equipo:"Peso corporal",logType:"bodyweight_reps",grupoSec:"Bíceps",patron:"Tracción",lateral:"Bilateral"},
  ...
  {nombre:"Elevaciónes laterales",grupo:"Hombros",tipo:"Fuerza",equipo:"Mancuerna",logType:"weight_reps",grupoSec:"",patron:"Aislamiento",lateral:"Bilateral"},
  ...
  {nombre:"Extensión de triceps en polea",grupo:"Tríceps",tipo:"Fuerza",equipo:"Polea",logType:"weight_reps",grupoSec:"",patron:"Aislamiento",lateral:"Bilateral"},
  ...
  {nombre:"Elevaciónes de piernas colgado",grupo:"Core",tipo:"Peso corporal",equipo:"Peso corporal",logType:"bodyweight_reps",grupoSec:"",patron:"Core",lateral:"Bilateral"},
...
const BACKUP_KEY_NAMES = ["tabs","cargas","medidas","gymPlanos","planWeek","objectives","rpe","snota","customFoods","supplements","nutProfile","mealPlan","shopping","proteinLog","recipes","nutGoal","trainingTemplate","exlib","gymTpl"];
...
function GymPanel({initPlanoKey=null,autoStart=false,onSessionStarted}){
  ...
  if(histEj){
    ...
    const _stats=(()=>{
      ...
      if(_lt==="cardio"){ let maxDist=0,longest=0,bestSessDist=0; _aggHist.forEach(e=>{ const ss=normSets(e.series,"cardio"); let td=0; ss.forEach(s=>{ if((s.distance||0)>maxDist)maxDist=s.distance; if((s.seconds||0)>longest)longest=s.seconds; td+=(s.distance||0); }); if(td>bestSessDist)bestSessDist=td; }); return {cardio:true,maxDist:Math.round(maxDist*100)/100,longest,bestSessDist:Math.round(bestSessDist*100)/100,has:maxDist>0||longest>0}; }
    ...
    const _statRows=_lt==="duration"
      ...
      : _lt==="cardio"
        ? [["Distancia máx",_stats.maxDist>0?(_stats.maxDist+"km"):"–"],["Tiempo máx",_stats.longest>0?fmtSecs(_stats.longest):"–"],["Mejor distancia/sesión",_stats.bestSessDist>0?(_stats.bestSessDist+"km"):"–"]]
      : _lt==="bodyweight_reps"
  ...
  if(showSummary){
    const items=plano.ejercicios.map(ej=>{
      ...
      return{ej,lt,setLine,headline,delta,isRecord};
    }).filter(Boolean);
    ...
    return <div>
      ...
      {items.map(({ej,lt,setLine,headline,delta,isRecord},i)=>(
        <div key={i} style={{padding:"12px 0",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            ...
            <div style={{textAlign:"right"}}>
              ...
              {delta!=null&&delta!==0&&<p style={{color:delta>0?C.green:C.red,fontSize:11,margin:0}}>{delta>0?"▲":"▼"}{lt==="weight_reps"?(Math.abs(delta)+"kg"):""}</p>}
            </div>
          </div>
        </div>
      ))}
