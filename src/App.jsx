  ...
  if(showSummary){
    ...
    return <div>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:14}}>
        ...
        <p style={{color:C.text,fontSize:14,fontWeight:600,margin:"0 0 6px",letterSpacing:-0.2}}>Plano {planoSel} · {plano.nombre}</p>
        ...
        {addingPlano
          ?<div style={{display:"flex",gap:4,alignItems:"center"}}>
             <input value={newPlanoName} onChange={e=>setNewPlanoName(e.target.value)} placeholder="D" maxLength={2} autoFocus
               ...
               style={{WebkitAppearance:"none",fontFamily:"inherit",width:36,background:C.surface,border:`1px solid ${C.accent}`,borderRadius:6,color:C.text,padding:"5px 8px",fontSize:11,outline:"none"}}/>
            ...
            {Array.from({length:ej.series},(_,si)=>{
              ...
              return <div key={si} style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>
                ...
                <input type="number" value={val}
                  ...
                  onBlur={()=>{}}
                  placeholder={lastSeries[si]?`${lastSeries[si]}kg`:"kg"} style={{background:confirmed?C.green+"18":C.surface,border:`1px solid ${confirmed?C.green+"55":C.border}`,borderRadius:8,color:C.text,padding:"6px 8px",fontSize:11,flex:1,boxSizing:"border-box",outline:"none",WebkitAppearance:"none",fontFamily:"inherit"}}/>
              </div>;
            })}
          </div>
...
function MedidasPanel(){
  ...
  if(mode==="add") return <div>
    <BackBtn onClick={()=>setMode("list")}/>
    <p style={{color:C.text,fontSize:14,fontWeight:600,margin:"0 0 14px"}}>Nueva evaluación</p>
    ...
    {todayAssignments.length===0?(
      ...
      <div style={{marginBottom:14}}>
        {todayAssignments.map((a,i)=>{
          ...
          return <div key={i} style={{background:C.card,border:`1px solid ${done?C.accent+"44":C.border}`,borderRadius:12,padding:14,marginBottom:i<todayAssignments.length-1?8:0}}>
      ...
      <Card style={{marginBottom:12}}>
        ...
        {showManual&&<div style={{background:C.surface,borderRadius:8,padding:"10px",marginBottom:8}}>
          <input value={manualName} onChange={e=>setManualName(e.target.value)} placeholder="Nombre (ej: Pollo + arroz)"
            style={{WebkitAppearance:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"8px 12px",fontSize:12,outline:"none",marginBottom:8}}/>
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            <div style={{flex:1}}>
              ...
              <input value={manualProt} onChange={e=>setManualProt(e.target.value)} type="number" inputMode="numeric" placeholder="0"
                style={{WebkitAppearance:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"6px 8px",fontSize:11,outline:"none"}}/>
            </div>
            <div style={{flex:1}}>
            ...
            {MEAL_TYPES.map((mt,mi)=>{
              ...
              return <div key={mi} style={{marginBottom:8}}>
                ...
                <input type="text" value={m.desc||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],desc:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                  placeholder="Descripción…" style={{WebkitAppearance:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box",background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 7px",fontSize:11,outline:"none",marginBottom:4}}/>
                <div style={{display:"flex",gap:5}}>
                  <input type="number" inputMode="numeric" value={m.prot||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],prot:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                    placeholder="Prot g" style={{WebkitAppearance:"none",fontFamily:"inherit",flex:1,minWidth:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"4px 6px",fontSize:11,outline:"none"}}/>
                  <input type="number" inputMode="numeric" value={m.kcal||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],kcal:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                    placeholder="Kcal" style={{WebkitAppearance:"none",fontFamily:"inherit",flex:1,minWidth:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"4px 6px",fontSize:11,outline:"none"}}/>
                </div>
              </div>;
            })}
          </div>}
            ...
            {items.map(item=>(
              <div key={item.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
                ...
                <input value={item.name} onChange={e=>saveShopping(shopping.map(s=>s.id===item.id?{...s,name:e.target.value}:s))}
                  style={{flex:1,background:"transparent",border:"none",color:item.done?C.textMuted:C.text,fontSize:11,padding:0,outline:"none",textDecoration:item.done?"line-through":"none",fontFamily:"inherit",WebkitAppearance:"none"}}/>
...
function AppInner(){
  ...
  useEffect(()=>{
    ...
    s.textContent="html,body{overflow:hidden;overscroll-behavior:none;height:100%;position:fixed;width:100%;}*{-webkit-overflow-scrolling:touch;}";
    ...
    return()=>{
      ...
      document.removeEventListener("touchmove",noZoom);
    };
  },[]);
