...
function CFPanel({tab,onDeleteTab,autoStart=false,onSessionStarted}){
  ...
  return <div>
    {confirm&&<ConfirmDialog msg={confirm.msg} onConfirm={confirm.onConfirm} onCancel={()=>setConfirm(null)}/>}
    <Card style={{marginBottom:16,background:C.surface,border:`1px solid ${C.border}`}}>
...
function MedidasPanel(){
  ...
  return <div>
    ...
    {medidas.length===0&&<p style={{color:C.textMuted,fontSize:13,textAlign:"center",padding:"20px 0"}}>Sin evaluaciones registradas</p>}
    {medidas.map((entry,i)=>{
        ...
        {isOpen&&<div style={{background:C.card,border:`1px solid ${C.border}`,borderTopWidth:0,borderRadius:"0 0 8px 8px",padding:"12px 14px"}}>
          ...
          {MEDIDAS_FIELDS.filter(f=>entry[f.id]!==undefined&&entry[f.id]!=="").map(f=>{
            ...
            return <div key={f.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
              ...
              <div style={{textAlign:"right"}}>
                <p style={{color:C.text,fontSize:15,fontWeight:600,margin:0}}>{entry[f.id]}{f.unit}</p>
  ...
  return <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"12px 14px",marginBottom:12}}>
    ...
    <div style={{display:"flex",gap:8,alignItems:"center"}}>
      ...
      <input
        ...
        placeholder="7.5"
        inputMode="decimal"
        style={{flex:1,minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,padding:"7px 10px",fontSize:12,outline:"none",WebkitAppearance:"none"}}
      />
...
function NutricionHoy({onNavigate,ydProtPct=100}){
  ...
  const [supplements,setSupplements]=useState(()=>load(K.supplements)||[{id:"sup1",name:"Creatina",dose:"5g"},{id:"sup2",name:"Batido de proteínas",dose:"30g"}]);
  ...
  function deleteSup(id){
    setSupplements(prev=>{
      ...
      save(K.supplements,upd);
      return upd;
    });
    saveSupDone({...supDone,[id]:false});
    saveProteinLog(proteinLog.filter(e=>e.supId!==id));
  }
  function addSup(){
    if(!newSupName.trim()) return;
    const pg=newSupIsProtein?parseInt(newSupProtGrams)||0:0;
    const ns={id:"sup_"+Date.now(),name:newSupName.trim(),dose:newSupDose.trim(),isProtein:newSupIsProtein,proteinGrams:pg};
    saveSupplements([...supplements,ns]);
    setNewSupName("");setNewSupDose("");setNewSupIsProtein(false);setNewSupProtGrams("");setAddingSup(false);
  }
  // Detect protein grams for a supplement — explicit field takes priority, falls back to legacy keyword+dose
  function getSupProteinGrams(sup){
    if(sup.isProtein===true) return sup.proteinGrams||0;
    if(sup.isProtein===false) return 0;
    // Legacy: detect by name keyword + parse dose
    const nameLC=(sup.name||"").toLowerCase();
    const isLegacyProtein=nameLC.includes("proteí")||nameLC.includes("batido")||nameLC.includes("whey")||nameLC.includes("caseína");
    if(!isLegacyProtein) return 0;
  ...
  return <Card style={{marginBottom:14}}>
    {/* Header — navigable */}
    <button onClick={onNavigate} style={{width:"100%",background:"none",border:"none",cursor:onNavigate?"pointer":"default",display:"flex",justifyContent:"space-between",alignItems:"center",padding:0,marginBottom:12}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <p style={{color:C.textMuted,fontSize:11,margin:0}}>{kcalObj} kcal</p>
        {onNavigate&&<span style={{color:C.textMuted,fontSize:14}}>›</span>}
      </div>
    </button>

    {/* Ayer: contextual line if protein was low */}
    {ydProtPct<60&&<p style={{color:C.orange,fontSize:11,margin:"-4px 0 10px",fontWeight:400}}>Ayer: {ydProtPct}% de proteína — dale caña hoy</p>}
    {/* BARS */}
      ...
      {supplements.map(sup=>(
        <div key={sup.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
          <button onClick={()=>{
            ...
            const pg=getSupProteinGrams(sup);
            if(pg>0){if(nowDone)saveProteinLog([...proteinLog,{name:sup.name,prot:pg,time:new Date().toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"}),supId:sup.id}]);
          ...
          <button onClick={()=>deleteSup(sup.id)} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:13,padding:"0 0 0 4px",lineHeight:1,flexShrink:0}}>✕</button>
        </div>
      ))}
      {addingSup
        ?<div style={{marginTop:10,width:"100%",boxSizing:"border-box",background:C.surface,borderRadius:8,padding:"10px 12px",border:`1px solid ${C.border}`}}>
           <div style={{display:"flex",gap:6,marginBottom:8,width:"100%",boxSizing:"border-box"}}>
             <input value={newSupName} onChange={e=>setNewSupName(e.target.value)} placeholder="Nombre"
               onKeyDown={e=>{if(e.key==="Enter")addSup();}}
               style={{flex:1,minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"7px 8px",fontSize:12,outline:"none"}}/>
             <input value={newSupDose} onChange={e=>setNewSupDose(e.target.value)} placeholder="Dosis"
               style={{flex:"0 0 72px",minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"7px 8px",fontSize:12,outline:"none"}}/>
           </div>
           <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:newSupIsProtein?8:10,width:"100%",boxSizing:"border-box"}}>
             <p style={{color:C.textMuted,fontSize:12,margin:0,flex:1,minWidth:0}}>¿Proteína?</p>
             <button onClick={()=>setNewSupIsProtein(v=>!v)}
               style={{flexShrink:0,background:newSupIsProtein?C.accent:C.bg,border:`1px solid ${newSupIsProtein?C.accent:C.border}`,borderRadius:6,color:newSupIsProtein?"#fff":C.textMuted,padding:"5px 12px",cursor:"pointer",fontSize:12,fontWeight:500}}>
               {newSupIsProtein?"Sí":"No"}
             </button>
           </div>
           {newSupIsProtein&&<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,width:"100%",boxSizing:"border-box"}}>
             <p style={{color:C.textMuted,fontSize:12,margin:0,flex:1,minWidth:0}}>g por toma</p>
             <input value={newSupProtGrams} onChange={e=>setNewSupProtGrams(e.target.value)} placeholder="25"
               type="number" inputMode="numeric"
               style={{flexShrink:0,width:64,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"7px 8px",fontSize:12,outline:"none"}}/>
           </div>}
           <div style={{display:"flex",gap:6,width:"100%",boxSizing:"border-box"}}>
             <button onClick={addSup} style={{flex:1,minWidth:0,background:C.accent,border:"none",borderRadius:6,color:"#fff",padding:"8px",cursor:"pointer",fontSize:12,fontWeight:600}}>Añadir</button>
             <button onClick={()=>{setAddingSup(false);setNewSupName("");setNewSupDose("");setNewSupIsProtein(false);setNewSupProtGrams("");}} style={{flexShrink:0,background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.textMuted,padding:"8px 10px",cursor:"pointer",fontSize:12}}>✕</button>
           </div>
         </div>
        :<button onClick={()=>setAddingSup(true)} style={{marginTop:8,background:"none",border:`1px dashed ${C.border}`,borderRadius:6,color:C.textMuted,padding:"7px 10px",cursor:"pointer",fontSize:11,width:"100%"}}>+ Añadir suplemento</button>
      }
...
function ScreenHoyMerged({onEmpezar,onGoToNutricion,onGoToSemana,onGoToFisico,onGoToComidas}){
  ...
  if(ydProtPct<60&&todayAssignments.length>0&&!dismissed.protLow){
    alerts.push({key:"protLow",color:C.orange,icon:"",title:`Proteína ayer · ${ydProtPct}%`,body:null});
  }
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:6,marginBottom:showSleep?8:14}}>
      ...
      {(()=>{const d=dayState[0];return <button onClick={()=>setShowSleep(s=>!s)} style={{flex:1,background:showSleep?C.card:C.surface,border:`1px solid ${showSleep?C.accent:C.border}`,borderRadius:8,padding:"8px 6px",textAlign:"center",cursor:"pointer",position:"relative"}}>
        ...
        <p style={{color:d.val==="—"?C.textMuted:C.text,fontSize:15,fontWeight:600,margin:0,lineHeight:1}}>{d.val}</p>
        <span style={{position:"absolute",right:4,top:"50%",transform:`translateY(-50%) rotate(${showSleep?180:0}deg)`,color:C.textMuted,fontSize:9,display:"inline-block",transition:"transform 0.2s"}}>▼</span>
      </button>;})()}
    ...
    {activeAlert&&<div style={{background:C.surface,border:`1px solid ${C.border}`,borderLeft:`2px solid ${activeAlert.color}`,borderRadius:8,padding:"8px 12px",marginBottom:10,display:"flex",gap:8,alignItems:"center"}}>
      <div style={{flex:1}}>
        <p style={{color:C.text,fontSize:12,fontWeight:500,margin:0,letterSpacing:-0.1}}>{activeAlert.title}</p>
        {activeAlert.body&&<p style={{color:C.textSub,fontSize:11,margin:"2px 0 0",lineHeight:1.4}}>{activeAlert.body}</p>}
      </div>
      <button onClick={()=>dismiss(activeAlert.key)} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:12,padding:0,flexShrink:0}}>✕</button>
    </div>}
...
function ScreenNutricion({initTab="hoy"}){
  const nutColor=C.accent;
  const [nutTab,setNutTab]=useState(initTab);
  ...
  function saveCustomFoods(upd){setCustomFoods(upd);save(K.customFoods,upd);}
  function saveSupplements(upd){setSupplements(upd);save(K.supplements,upd);}
  function deleteSup(id){setSupplements(prev=>{const u=prev.filter(s=>s.id!==id);save(K.supplements,u);return u;});saveSupDone({...supDone,[id]:false});saveProteinLog(proteinLog.filter(e=>e.supId!==id));}
  ...
  const last7=useMemo(()=>[...Array(7)].map((_,i)=>{
    const d=new Date();d.setDate(d.getDate()-i);
    const ds=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    const p=((load(K.proteinLog)||{})[ds]||[]).reduce((a,e)=>a+(e.prot||0),0);
    const w=(load(K.proteinLog)||{})[ds+"_h"]||0;
    return {prot:p>=pt,water:w>=ht};
  }).reverse(),[]);
  const daysHit=last7.filter(d=>d.prot).length;
  const waterDaysHit=last7.filter(d=>d.water).length;

  const TABS=[["hoy","Hoy"],["semana","Semana"],["plan","Plan"],["compra","Compra"]];

  return <div>
    {/* Compact adherence strip + kcal target */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{display:"flex",gap:3,alignItems:"center"}}>
        {last7.map((d,i)=><div key={i} style={{width:8,height:8,borderRadius:"50%",background:d.prot?C.green:C.borderLight}}/>)}
        <p style={{color:C.textMuted,fontSize:10,margin:"0 0 0 5px"}}>{daysHit}/7</p>
      </div>
    ...
    <div style={{display:"flex",gap:0,marginBottom:14,background:C.bg,borderRadius:10,padding:3,border:`1px solid ${C.border}`}}>
      {TABS.map(([id,label])=>(
        <button key={id} onClick={()=>setNutTab(id)} style={{flex:1,background:nutTab===id?C.card:"transparent",color:nutTab===id?C.text:C.textMuted,border:nutTab===id?`1px solid ${C.border}`:"1px solid transparent",borderRadius:8,padding:"7px 4px",fontSize:11,fontWeight:nutTab===id?600:400,cursor:"pointer",transition:"all 0.12s"}}>{label}</button>
      ))}
    </div>

    {/* ── HOY ── */}
    {nutTab==="hoy"&&<Card style={{marginBottom:0}}>
      {/* Summary row — protein + water compact */}
      <div style={{display:"flex",gap:0,marginBottom:10}}>
        <div style={{flex:1,paddingRight:12,borderRight:`1px solid ${C.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:4}}>
            ...
            <p style={{color:protDone?C.green:C.textMuted,fontSize:10,fontWeight:600,margin:0}}>{protDone?"✓":pt-ph+"g"}</p>
          </div>
          <p style={{color:C.text,fontSize:17,fontWeight:700,margin:"0 0 5px",lineHeight:1,letterSpacing:-0.3}}>{ph}<span style={{color:C.textMuted,fontSize:11,fontWeight:400}}>/{pt}g</span></p>
          <ProgressBar pct={Math.min(100,Math.round((ph/pt)*100))} color={protDone?C.green:ph/pt>=0.7?C.orange:nutColor} height={4}/>
        </div>
        <div style={{flex:1,paddingLeft:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:4}}>
            <p style={{color:C.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:1,margin:0}}>Agua{isWOD?" WOD":""}</p>
            <p style={{color:watDone?C.green:C.textMuted,fontSize:10,fontWeight:600,margin:0}}>{watDone?"✓":Math.round((wtgt-hydration)*10)/10+"L"}</p>
          </div>
          <p style={{color:C.text,fontSize:17,fontWeight:700,margin:"0 0 5px",lineHeight:1,letterSpacing:-0.3}}>{hydration}<span style={{color:C.textMuted,fontSize:11,fontWeight:400}}>/{wtgt}L</span></p>
          <ProgressBar pct={Math.min(100,Math.round((hydration/wtgt)*100))} color={watDone?C.green:C.blue} height={4}/>
        </div>
      </div>

      {/* Water quick-add inline */}
      <div style={{display:"flex",gap:4,paddingBottom:10,borderBottom:`1px solid ${C.border}`,marginBottom:10}}>
      ...
      {/* Protein log header */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <p style={{color:C.text,fontSize:12,fontWeight:600,margin:0}}>Proteína del día</p>
        <div style={{display:"flex",gap:8}}>
          {ph>0&&<button onClick={()=>saveProteinLog([])} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:10,padding:0}}>↺</button>}
          <button onClick={()=>setShowAdd(s=>!s)}
            style={{background:"none",border:`1px solid ${showAdd?C.accent:C.border}`,color:showAdd?C.accent:C.textSub,borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:10,fontWeight:500}}>+ Añadir</button>
        </div>
      </div>

      {showAdd&&<div style={{background:C.surface,borderRadius:8,padding:8,marginBottom:8}}>
        <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:6}}>
          {allFoods.map(f=><button key={f.name||f.id} onClick={()=>{saveProteinLog([...proteinLog,{name:f.name,prot:f.prot,time:new Date().toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"})}]);setShowAdd(false);}}
            style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:10,color:C.textSub}}>+{f.prot}g {f.name}</button>)}
        </div>
        <CustomProteinAdd onAdd={e=>{saveProteinLog([...proteinLog,e]);setShowAdd(false);}} color={nutColor} onSaveFood={f=>saveCustomFoods([...customFoods,f])}/>
      </div>}

      {proteinLog.length===0
        ?<p style={{color:C.textMuted,fontSize:11,margin:"0 0 10px",paddingBottom:10,borderBottom:`1px solid ${C.border}`}}>Sin registros hoy</p>
        :<div style={{paddingBottom:10,borderBottom:`1px solid ${C.border}`,marginBottom:0}}>
          {proteinLog.slice().reverse().map((e,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderTop:i>0?`1px solid ${C.border}`:"none"}}>
              <p style={{color:C.textSub,fontSize:12,margin:0}}>{e.name}{e.time&&<span style={{color:C.textMuted,fontSize:10}}> · {e.time}</span>}</p>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <p style={{color:nutColor,fontSize:11,fontWeight:700,margin:0}}>+{e.prot}g</p>
                <button onClick={()=>{const u=[...proteinLog];u.splice(proteinLog.length-1-i,1);saveProteinLog(u);}} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:10,padding:0}}>✕</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Supplements — compact, no header card */}
      <div style={{marginTop:10}}>
        <p style={{color:C.textMuted,fontSize:10,letterSpacing:1.2,textTransform:"uppercase",margin:"0 0 8px",fontWeight:500}}>Suplementos</p>
        {supplements.map(sup=>(
          <div key={sup.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderTop:`1px solid ${C.border}`}}>
            <button onClick={()=>{const nowDone=!supDone[sup.id];saveSupDone({...supDone,[sup.id]:nowDone});const pg=getSupPg(sup);if(pg>0){if(nowDone)saveProteinLog([...proteinLog,{name:sup.name,prot:pg,time:new Date().toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"}),supId:sup.id}]);else saveProteinLog(proteinLog.filter(e=>e.supId!==sup.id));}}}
              style={{width:18,height:18,borderRadius:4,flexShrink:0,background:supDone[sup.id]?C.green:C.surface,border:`2px solid ${supDone[sup.id]?C.green:C.borderLight}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:10,fontWeight:700}}>{supDone[sup.id]?"✓":""}</button>
        ...
        {addingSup
          ?<div style={{marginTop:8,width:"100%",boxSizing:"border-box",background:C.surface,borderRadius:8,padding:"10px 10px",border:`1px solid ${C.border}`}}>
             <div style={{display:"flex",gap:6,marginBottom:8}}>
               <input value={newSupName} onChange={e=>setNewSupName(e.target.value)} placeholder="Nombre" onKeyDown={e=>{if(e.key==="Enter")addSup();}}
                 style={{flex:1,minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"6px 8px",fontSize:12,outline:"none"}}/>
               <input value={newSupDose} onChange={e=>setNewSupDose(e.target.value)} placeholder="Dosis"
                 style={{flex:"0 0 66px",minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"6px 8px",fontSize:12,outline:"none"}}/>
             </div>
             ...
             {newSupIsProtein&&<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
               ...
               <input value={newSupProtGrams} onChange={e=>setNewSupProtGrams(e.target.value)} placeholder="25" type="number" inputMode="numeric"
                 style={{width:60,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"6px 8px",fontSize:12,outline:"none"}}/>
             </div>}
             <div style={{display:"flex",gap:6}}>
               ...
               <button onClick={()=>{setAddingSup(false);setNewSupName("");setNewSupDose("");setNewSupIsProtein(false);setNewSupProtGrams("");}}
                 style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.textMuted,padding:"7px 10px",cursor:"pointer",fontSize:12}}>✕</button>
             </div>
           </div>
          :<button onClick={()=>setAddingSup(true)}
             style={{marginTop:6,background:"none",border:`1px dashed ${C.border}`,borderRadius:6,color:C.textMuted,padding:"5px 10px",cursor:"pointer",fontSize:10,width:"100%"}}>+ Añadir suplemento</button>
        }
      </div>
    </Card>}

    {/* ── SEMANA ── simple adherence only */}
    {nutTab==="semana"&&<div>
      <Card style={{marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <p style={{color:C.text,fontSize:13,fontWeight:600,margin:0}}>Proteína</p>
          <p style={{color:daysHit>=5?C.green:daysHit>=3?C.orange:C.red,fontSize:12,fontWeight:700,margin:0}}>{daysHit}/7 días</p>
        </div>
        <div style={{display:"flex",gap:4,marginBottom:4}}>
          {last7.map((d,i)=><div key={i} style={{flex:1,height:6,borderRadius:4,background:d.prot?C.green:C.borderLight}}/>)}
        </div>
        <p style={{color:C.textMuted,fontSize:10,margin:0}}>{daysHit>=5?"Excelente":daysHit>=3?"Mejorable — objetivo 5+ días":"Baja — prioriza proteína esta semana"}</p>
      </Card>
      <Card style={{marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <p style={{color:C.text,fontSize:13,fontWeight:600,margin:0}}>Agua</p>
          <p style={{color:waterDaysHit>=5?C.green:waterDaysHit>=3?C.orange:C.red,fontSize:12,fontWeight:700,margin:0}}>{waterDaysHit}/7 días</p>
        </div>
        <div style={{display:"flex",gap:4}}>
          {last7.map((d,i)=><div key={i} style={{flex:1,height:6,borderRadius:4,background:d.water?C.blue:C.borderLight}}/>)}
        </div>
      </Card>
      <Card>
        <div style={{display:"flex",gap:10}}>
          {[["Proteína",pt+"g"],["Agua",ht+"L"],["Kcal",kcalObj+""]].map(([l,v])=>(
            <div key={l} style={{flex:1,textAlign:"center"}}>
              <p style={{color:C.textMuted,fontSize:10,margin:"0 0 3px"}}>{l}</p>
              <p style={{color:C.text,fontSize:14,fontWeight:700,margin:0}}>{v}</p>
            </div>
          ))}
        </div>
      </Card>
    ...
    {/* ── PLAN — today expanded, rest collapsed ── */}
    {nutTab==="plan"&&<div>
      {/* Pre/post workout — single compact line */}
      ...
      {orderedDays.map((di)=>{
        ...
        return <div key={di} style={{marginBottom:6}}>
          ...
          {isExpanded&&<div style={{background:C.card,border:`1px solid ${C.border}`,borderTopWidth:0,borderRadius:"0 0 8px 8px",padding:"8px 10px"}}>
            {MEAL_TYPES.map((mt,mi)=>{
              const val=(mealPlan[di]?.meals||[])[mi]?.desc||"";
              const protTag=(mealPlan[di]?.meals||[])[mi]?.protTag||"";
              return <div key={mi} style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                <p style={{color:C.textMuted,fontSize:10,margin:0,width:78,flexShrink:0}}>{mt}</p>
                <input type="text" value={val} onChange={e=>{const upd={...mealPlan};const ms=upd[di]?.meals||MEAL_TYPES.map(t=>({t,desc:"",protTag:""}));upd[di]={...upd[di],meals:ms.map((m,idx)=>idx===mi?{...m,desc:e.target.value}:m)};saveMealPlan(upd);}}
                  placeholder="Añadir..." style={{flex:1,minWidth:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 8px",fontSize:11,outline:"none"}}/>
                <div style={{display:"flex",gap:2}}>
                  {[["P","P"],["C","C"],["B","M"]].map(([icon,code])=>(
                    <button key={code} onClick={()=>{const upd={...mealPlan};const ms=upd[di]?.meals||MEAL_TYPES.map(t=>({t,desc:"",protTag:""}));upd[di]={...upd[di],meals:ms.map((m,idx)=>idx===mi?{...m,protTag:protTag===code?"":code}:m)};saveMealPlan(upd);}}
                      style={{background:protTag===code?C.accent+"22":C.surface,border:`1px solid ${protTag===code?C.accent:C.border}`,borderRadius:4,padding:"3px 5px",cursor:"pointer",fontSize:9,color:protTag===code?C.accent:C.textMuted,fontWeight:protTag===code?600:400}}>{icon}</button>
                  ))}
                </div>
              </div>;
            })}
    ...
    {nutTab==="compra"&&<div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <div style={{display:"flex",gap:6}}>
          <button onClick={()=>saveShopping(shopping.map(s=>({...s,done:false})))} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.textSub,padding:"4px 8px",cursor:"pointer",fontSize:10}}>Resetear</button>
          <button onClick={()=>saveShopping([])} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.textMuted,padding:"4px 8px",cursor:"pointer",fontSize:10}}>Limpiar</button>
        </div>
      </div>
            ...
            {items.map(item=>(
              <div key={item.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
                <button onClick={()=>saveShopping(shopping.map(s=>s.id===item.id?{...s,done:!s.done}:s))}
                  style={{width:20,height:20,borderRadius:5,background:item.done?C.green:C.surface,border:`2px solid ${item.done?C.green:C.borderLight}`,cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,fontWeight:700}}>{item.done?"✓":""}</button>
                <input value={item.name} onChange={e=>saveShopping(shopping.map(s=>s.id===item.id?{...s,name:e.target.value}:s))}
                  style={{flex:1,background:"transparent",border:"none",color:item.done?C.textMuted:C.text,fontSize:12,padding:0,outline:"none",textDecoration:item.done?"line-through":"none",fontFamily:"inherit"}}/>
