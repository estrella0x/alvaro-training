    ...
    if(this.state.error){
      ...
      return (
        <div style={{padding:40,textAlign:"center",fontFamily:"-apple-system,sans-serif",background:"#0b0b13",minHeight:"100vh"}}>
          ...
          <p style={{fontSize:16,fontWeight:700,color:"#f43f5e",margin:"0 0 8px"}}>Algo salió mal</p>
          <p style={{fontSize:12,color:"#9292b2",margin:"0 0 24px",maxWidth:300,marginLeft:"auto",marginRight:"auto"}}>{msg}</p>
          <button onClick={()=>window.location.reload()} style={{background:"#f43f5e",color:"#fff",border:"none",borderRadius:10,padding:"10px 24px",cursor:"pointer",fontSize:14,fontWeight:700}}>Recargar app</button>
...
const DARK = {
  ...
  // Single accent
  accent:  "#0a7aff",
  // Status — used only for actual states
  green:   "#34c759",
  red:     "#ff3b30",
  orange:  "#ff9500",
  blue:    "#0a84ff",
...
const DEFAULT_LIBRE = [
  ...
  {id:"l4",icon:"",name:"Slackline",               detail:"10-15min. Propiocepción tobillo/rodilla.",dia:"Libre"},
  ...
  Object.entries(cargas).forEach(([ejId,hist])=>{
    ...
    hist.forEach(e=>{
      ...
      if(!byDate[e.fecha].has("gym_"+info.key))
        byDate[e.fecha].set("gym_"+info.key,{tabId:"gym_"+info.key,tabName:`Gym — Plano ${info.key}`,icon:"",color:info.color,planoKey:info.key});
    });
  });
...
function Card({children,style={},onClick,accent,raised=false}){
  return <div onClick={onClick} style={{background:C.card,border:`1px solid ${accent?accent+"44":C.border}`,borderLeft:accent?`2px solid ${accent}`:`1px solid ${C.border}`,borderRadius:12,padding:16,cursor:onClick?"pointer":"default",...style}}>{children}</div>;
}
function Btn({children,onClick,color,outline=false,style={}}){
  const bg=color||C.accent;
  return <button onClick={onClick} style={{background:outline?"transparent":bg,color:outline?bg:"#fff",border:`1px solid ${outline?bg+"88":bg}`,borderRadius:8,padding:"10px 16px",fontSize:13,fontWeight:600,cursor:"pointer",letterSpacing:-0.1,...style}}>{children}</button>;
}
function Input({value,onChange,type="number",placeholder,style={},onEnter}){
  return <input
    type={type} value={value} onChange={onChange} placeholder={placeholder}
    onKeyDown={onEnter?e=>{if(e.key==="Enter"){e.preventDefault();onEnter();}}:undefined}
    style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,padding:"10px 12px",fontSize:14,width:"100%",boxSizing:"border-box",outline:"none",transition:"border-color 0.15s",...style}}
  />;
}
function Tag({children,color}){
  return <span style={{background:color+"1a",color,fontSize:10,letterSpacing:0.5,padding:"3px 8px",borderRadius:5,fontWeight:600,display:"inline-flex",alignItems:"center",fontVariantNumeric:"tabular-nums"}}>{children}</span>;
}
function BackBtn({onClick}){return <button onClick={onClick} style={{background:"none",border:"none",color:C.textSub,cursor:"pointer",fontSize:13,marginBottom:14,padding:0,letterSpacing:-0.1}}>← Volver</button>;}
function SectionLabel({children,style={}}){return <p style={{color:C.textMuted,fontSize:10,letterSpacing:1.5,textTransform:"uppercase",margin:"0 0 10px",fontWeight:500,...style}}>{children}</p>;}
function SavedBadge({color}){return <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,padding:"7px 12px",marginBottom:10,color:C.textSub,fontSize:11,fontWeight:500}}>Guardado</div>;}
function ProgressBar({pct,color,height=4}){return <div style={{background:C.border,borderRadius:height,height,overflow:"hidden"}}><div style={{background:color,borderRadius:height,height,width:`${Math.min(100,pct)}%`,transition:"width 0.5s ease"}}/></div>;}
function InsightBox({data,unit}){
  const ins=getInsight(data,unit);
  if(!ins) return null;
  const c={success:C.green,warn:C.orange,info:C.blue}[ins.type]||C.textMuted;
  const sym={success:"↑",warn:"—",info:"·"}[ins.type]||"·";
  return <div style={{marginTop:10,padding:"8px 12px",background:C.surface,borderRadius:6,borderLeft:`2px solid ${c}`}}><p style={{color:c,fontSize:11,margin:0,fontWeight:500}}>{sym} {ins.text}</p></div>;
}
...
function ConfirmDialog({msg,onConfirm,onCancel}){
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"0 0 20px"}}>
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:20,maxWidth:480,width:"calc(100% - 32px)"}}>
...
function RestTimer({seconds,onClose}){
  ...
  return <div style={{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 20px",zIndex:200,minWidth:220}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
      <p style={{color:C.textMuted,fontSize:10,letterSpacing:1,textTransform:"uppercase",margin:0,fontWeight:500}}>Descanso</p>
      <button onClick={onClose} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:14,padding:0}}>✕</button>
    </div>
    <p style={{color:remaining<=10?C.red:C.text,fontSize:40,fontWeight:700,margin:"0 0 10px",textAlign:"center",fontFamily:"-apple-system,monospace",letterSpacing:-1,transition:"color 0.3s"}}>
...
function SessionTimer({color,autoStart=false,onStarted}){
  ...
  return <button onClick={toggle} style={{background:active?color+"22":C.surface,border:`1px solid ${active?color:C.border}`,borderRadius:8,padding:"5px 12px",cursor:"pointer",color:active?color:C.textMuted,fontSize:12,fontWeight:active?700:400}}>
    ...
    return <div>
      ...
      <Card accent={color} style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <button onClick={()=>deleteExercise(detailId)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.red,padding:"6px 10px",cursor:"pointer",fontSize:12}}> Eliminar</button>
        </div>
                ...
                <div>
                  ...
                  {h.wod&&(()=>{
                    ...
                    return <div style={{marginTop:3}}>
                      {tipo&&<p style={{color:C.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:1,margin:"0 0 2px"}}>{tipo}</p>}
                      <p style={{color,fontSize:12,fontWeight:700,margin:0}}>{resultado}</p>
      ...
      return <Card style={{marginBottom:14,background:color+"0a",border:`1px solid ${color}33`}}>
        ...
        {results.map((r,i)=>(
          <div key={i} style={{padding:"7px 0",borderBottom:i<results.length-1?`1px solid ${C.border}`:"none"}}>
            <p style={{color:C.textSub,fontSize:11,margin:"0 0 3px"}}>{fmt(r.fecha)}</p>
            {(()=>{
              ...
              const resultado=colonIdx>=0?mainPart.slice(colonIdx+1).trim():mainPart;
              return <div>
                {tipo&&<p style={{color:C.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:1,margin:"0 0 1px"}}>{tipo}</p>}
                <p style={{color,fontSize:13,fontWeight:700,margin:0}}>{resultado}</p>
                {rpePart&&<p style={{color:C.textMuted,fontSize:10,margin:"2px 0 0"}}>{rpePart}</p>}
              </div>;
            })()}
          </div>
...
function GymPanel({initPlanoKey=null,autoStart=false,onSessionStarted}){
  ...
  const plano=planos[planoSel]||Object.values(planos)[0]||{ejercicios:[],color:C.gymColor,nombre:"Plano"};
    ...
    {safeArr(plano.ejercicios).map((ej,idx)=>{
      ...
      return <div key={ej.id} style={{background:done?C.gymColor+"12":C.card,border:`1px solid ${done?C.gymColor+"35":C.border}`,borderRadius:16,padding:14,marginBottom:10,transition:"all 0.2s"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
          ...
          <div style={{textAlign:"right"}}>
            {done?<p style={{color:C.gymColor,fontSize:13,fontWeight:700,margin:0}}>{safeMax(done,0)}kg</p>:last?<p style={{color:plano.color,fontSize:13,fontWeight:700,margin:0}}>{last.kg}kg</p>:null}
          </div>
        </div>
        ...
        {editMode&&<div style={{display:"flex",gap:6,marginBottom:8,borderTop:`1px solid ${C.border}`,paddingTop:8}}>
          ...
          <button onClick={()=>deleteEj(ej.id)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.red,padding:"4px 10px",cursor:"pointer",fontSize:12,marginLeft:"auto"}}> Eliminar</button>
        </div>}
...
const MEDIDAS_FIELDS=[
  {id:"peso",label:"Peso",unit:"kg",icon:""},{id:"grasa",label:"% Grasa",unit:"%",icon:""},
  ...
  return <div>
    ...
    {medidas.length>0&&<>
      <Card accent={C.gymColor} style={{marginBottom:14}}>
        ...
        {medidas.length===1&&<p style={{color:C.orange,fontSize:11,margin:"4px 0 0",fontStyle:"italic"}}>
...
function NutricionHoy(){
  ...
  return <Card style={{marginBottom:14}}>
    ...
    <TrackerRow icon="" label="Proteína" cur={ph} tgt={pt} unit="g" color={nutColor} onReset={()=>saveProteinLog([])}
      addButtons={<button onClick={()=>setShowProteinAdd(s=>!s)} style={{flex:1,background:showProteinAdd?C.accent+"22":C.surface,color:showProteinAdd?C.accent:C.textSub,border:`1px solid ${showProteinAdd?C.accent:C.border}`,borderRadius:8,padding:"7px 6px",cursor:"pointer",fontSize:11,fontWeight:600}}>+ Añadir</button>}/>
  ...
  return <div>
    ...
    {todayAssignments.length===0?(
      ...
      <div style={{marginBottom:14}}>
        {todayAssignments.map((a,i)=>{
          ...
          return <div key={i} style={{background:C.card,border:`1.5px solid ${done?color+"66":color+"33"}`,borderRadius:18,padding:18,marginBottom:i<todayAssignments.length-1?10:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:done?6:12}}>
              <div>
                <p style={{color:C.textMuted,fontSize:10,letterSpacing:0.5,margin:"0 0 4px",textTransform:"uppercase"}}>Hoy</p>
    ...
    <div style={{display:"flex",gap:6,marginBottom:14}}>
      {dayState.map(({label,val,color,ok})=>(
        <div key={label} style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 6px",textAlign:"center"}}>
          <p style={{color:C.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:0.5,margin:"0 0 4px",fontWeight:500}}>{label}</p>
          <p style={{color:ok?C.text:val==="—"?C.textMuted:color,fontSize:15,fontWeight:600,margin:0,lineHeight:1}}>{val}</p>
        </div>
      ))}
      <div style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 6px",textAlign:"center"}}>
        <p style={{color:C.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:0.5,margin:"0 0 4px",fontWeight:500}}>Semana</p>
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:14}}>
      <Card style={{flex:1,textAlign:"center",padding:12}}>
        <p style={{color:C.textMuted,fontSize:9,margin:"0 0 3px",textTransform:"uppercase",letterSpacing:1}}>Entrenados</p>
        <p style={{color:C.gymColor,fontSize:20,fontWeight:700,margin:0}}>{diasEntrenados}</p>
      </Card>
      <Card style={{flex:1,textAlign:"center",padding:12}}>
        <p style={{color:C.textMuted,fontSize:9,margin:"0 0 3px",textTransform:"uppercase",letterSpacing:1}}>Descansos</p>
        <p style={{color:C.orange,fontSize:20,fontWeight:700,margin:0}}>{diasEnMes-diasEntrenados}</p>
      </Card>
      <Card style={{flex:1,textAlign:"center",padding:12}}>
        <p style={{color:C.textMuted,fontSize:9,margin:"0 0 3px",textTransform:"uppercase",letterSpacing:1}}>% adherencia</p>
        <p style={{color:diasEntrenados/diasEnMes>=0.6?C.green:C.orange,fontSize:20,fontWeight:700,margin:0}}>
    ...
    <Card style={{marginBottom:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:6}}>
        {["L","M","X","J","V","S","D"].map(d=>(
          <p key={d} style={{color:C.textMuted,fontSize:9,textAlign:"center",margin:0,fontWeight:700}}>{d}</p>
        ))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
        ...
        {Array.from({length:diasEnMes},(_,i)=>i+1).map(day=>{
          ...
          const colors=safeArr(sessions).map(s=>s.color||C.gymColor);
          return <button
            key={day}
            ...
            style={{
              aspectRatio:"1",
              borderRadius:8,
              border:`1.5px solid ${isSelected?C.gymColor:isToday?"#ffffff44":"transparent"}`,
    ...
    {/* Day detail panel */}
    {selectedDay&&(
      <Card style={{border:`1.5px solid ${C.gymColor}33`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          selSessions.map((s,i)=>{
            ...
            return <div key={i} style={{marginBottom:i<selSessions.length-1?10:0,paddingBottom:i<selSessions.length-1?10:0,borderBottom:i<selSessions.length-1?`1px solid ${C.border}`:"none"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:nota?4:0}}>
                <p style={{color:s.color||C.gymColor,fontSize:13,fontWeight:700,margin:0}}>
        ...
        {(selProt>0||selSleep.rating>0)&&(
          <div style={{display:"flex",gap:8,marginTop:10,paddingTop:10,borderTop:`1px solid ${C.border}`}}>
            {selProt>0&&(
              <Tag color="#f97316"> {selProt}g prot</Tag>
            )}
            {selSleep.rating>0&&(
...
function ProgresoSemana(){
  ...
  return <div>
    ...
    {(()=>{
      ...
      const adherenceTxt=planVsReal.adherence===null?"Sin planificación activa":
        planVsReal.adherence>=100?"✓ Plan completado al 100%":
        planVsReal.adherence>=75?`✓ ${planVsReal.adherence}% del plan completado`:
        planVsReal.adherence>=50?`${planVsReal.adherence}% del plan completado`:
        ` ${planVsReal.adherence}% del plan completado`;

      return <div style={{background:C.surface,borderRadius:16,padding:16,marginBottom:16}}>
        <p style={{color:C.text,fontSize:13,fontWeight:700,margin:"0 0 10px"}}>Esta semana</p>

        {/* Plan vs ejecutado */}
        {planVsReal.planned>0&&<div style={{marginBottom:10,paddingBottom:10,borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
            <p style={{color:C.textSub,fontSize:12,margin:0}}>Planificado</p>
            <p style={{color:C.textSub,fontSize:12,fontWeight:700,margin:0}}>{planVsReal.executed}/{planVsReal.planned} sesiones</p>
          </div>
          <ProgressBar pct={planVsReal.adherence||0} color={planVsReal.adherence>=100?C.green:planVsReal.adherence>=75?C.gymColor:planVsReal.adherence>=50?C.orange:C.red} height={5}/>
          <p style={{color:planVsReal.adherence>=75?C.green:planVsReal.adherence>=50?C.orange:C.red,fontSize:11,margin:"4px 0 0"}}>{adherenceTxt}</p>
          {planVsReal.missed.length>0&&<p style={{color:C.textMuted,fontSize:10,margin:"3px 0 0"}}>Sin entrenar: {planVsReal.missed.join(", ")}</p>}
        </div>}
        ...
        {recos.length>0&&<div style={{borderTop:`1px solid ${C.border}`,paddingTop:8,marginBottom:8}}>
          {recos.map((r,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:i<recos.length-1?5:0}}>
            <span style={{color:C.gymColor,fontSize:11,flexShrink:0,marginTop:1}}>→</span>
        ...
        <div style={{display:"flex",gap:12,flexWrap:"wrap",paddingTop:conclusion||recos.length>0?8:0,borderTop:conclusion||recos.length>0?`1px solid ${C.border}`:"none"}}>
          {[
            ...
            <div key={label} style={{flex:1,minWidth:80}}>
              <p style={{color:C.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:1,margin:"0 0 2px"}}>{label}</p>
              <p style={{color:C.text,fontSize:18,fontWeight:800,margin:"0 0 1px",lineHeight:1}}>{cur}{unit}</p>
              {prev!=null&&(()=>{
                ...
                return <p style={{color:better?C.green:C.orange,fontSize:10,margin:0}}>{diff>0?"▲":"▼"} {Math.abs(diff).toFixed(label==="RPE medio"?1:0)}{unit}</p>;
              })()}
            </div>
          ))}
        </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
      {[{label:"Días",val:daysThisWeek,unit:"/ 7",status:daysThisWeek>=4?C.green:daysThisWeek>=2?C.orange:null},{label:"RPE",val:avgRPE||"—",unit:"/10",status:avgRPE>=8?C.red:avgRPE>=6?null:null},{label:"Proteína",val:daysHit,unit:"/ 7",status:daysHit>=5?C.green:daysHit>=3?C.orange:C.red}].map(({label,val,unit,status})=>(
        <Card key={label} style={{padding:"12px 10px",textAlign:"center"}}>
          <p style={{color:C.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:0.8,margin:"0 0 4px",fontWeight:500}}>{label}</p>
          <p style={{color:C.text,fontSize:20,fontWeight:700,margin:"0 0 3px",lineHeight:1,letterSpacing:-0.5}}>{val}</p>
          <p style={{color:C.textMuted,fontSize:9,margin:0}}>{unit}</p>
          {status&&<div style={{width:5,height:5,borderRadius:"50%",background:status,margin:"5px auto 0"}}/>}
        </Card>
      ))}
    </div>
    <Card style={{marginBottom:14}}>
    ...
    {weekPRs.length>0&&<Card style={{marginBottom:14,background:C.surface,border:`1px solid ${C.border}`}}>
...
function ProgresoCuerpo(){
  ...
  return <div>
    <div style={{background:C.card,border:`1px solid #f9731644`,borderRadius:20,padding:18,marginBottom:14}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>
        {[["Proteína Prot",proteinTarget+"g",C.red],["Carbos Carbos",carbTarget+"g",C.yellow],[" Grasa",fatTarget+"g",C.blue]].map(([l,v,c])=>(
    ...
    {gainRate!==null?<Card style={{marginBottom:14}}>
      ...
      <p style={{color:gainOk?C.green:gainSlow?C.orange:C.red,fontSize:13,fontWeight:700,margin:"0 0 4px"}}>
...
function ProgresoPlantificar(){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
      <p style={{color:C.text,fontSize:13,fontWeight:700,margin:0,flex:1}}>Planificar semana</p>
      <button onClick={handleSaveTemplate} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.textSub,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:600}}> Guardar plantilla</button>
      {templates.length>0&&<button onClick={()=>setShowTpl(s=>!s)} style={{background:showTpl?C.gymColor+"22":C.surface,border:`1px solid ${showTpl?C.gymColor:C.border}`,borderRadius:8,color:showTpl?C.gymColor:C.textSub,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:600}}> {templates.length}</button>}
    </div>
    {showTpl&&<div style={{background:C.surface,borderRadius:12,padding:12,marginBottom:14}}>
      ...
      {templates.map(tpl=>(
        <div key={tpl.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${C.border}`}}>
          <div>
            <p style={{color:C.text,fontSize:12,fontWeight:600,margin:0}}>{tpl.label}</p>
            <p style={{color:C.textMuted,fontSize:10,margin:0}}>Guardada el {tpl.saved}</p>
          </div>
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>handleApplyTemplate(tpl)} style={{background:C.gymColor,color:"#000",border:"none",borderRadius:8,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:700}}>Aplicar</button>
            <button onClick={()=>saveTemplates(templates.filter(t=>t.id!==tpl.id))} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.red,padding:"5px 8px",cursor:"pointer",fontSize:11}}>✕</button>
          </div>
        </div>
      ))}
    </div>}
...
const CAT_LABELS={proteina:"Proteína Proteína",lacteos:"Lácteos Lácteos",carbos:"Carbos Carbos",legumbres:"Legumbres Legumbres",verduras:"Verduras Verduras",frutas:"Frutas Frutas",condimentos:"Condimentos Condimentos",otros:"Otros Otros"};
...
function ScreenNutricion(){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:0,marginBottom:18,background:C.surface,borderRadius:8,padding:2,border:`1px solid ${C.border}`}}>
      {[["comidas","Comidas"],["compra","Compra"]].map(([id,label])=>(
        <button key={id} onClick={()=>setNutTab(id)} style={{flex:1,background:nutTab===id?C.card:"transparent",color:nutTab===id?C.text:C.textMuted,border:nutTab===id?`1px solid ${C.border}`:"1px solid transparent",borderRadius:6,padding:"8px 10px",fontSize:12,fontWeight:nutTab===id?600:400,cursor:"pointer",transition:"all 0.12s",letterSpacing:-0.2}}>{label}</button>
      ))}
    </div>
    {nutTab==="comidas"&&<div>
      {!lastTrainedToday&&loadWeek()[(new Date().getDay()+6)%7]?.assignments?.length>0&&(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",gap:10}}>
      ...
      {lastTrainedToday&&<div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",gap:10}}>
          ...
          {MEAL_TYPES.map((mt,mi)=>{
            ...
            return <div key={mi} style={{marginBottom:7}}>
              ...
              <div style={{display:"flex",gap:5,alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:3}}>
                  {[["Proteína","P"],["Carbos","C"],["M","M"]].map(([icon,code])=>(
    ...
    {nutTab==="compra"&&<div>
      ...
      <div style={{background:C.surface,borderRadius:12,padding:"10px 12px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          <button onClick={()=>saveShopping([])} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.red,padding:"5px 10px",cursor:"pointer",fontSize:11}}>Limpiar</button>
        </div>
      </div>
...
function BackupPanel({onClose,setImportMsg}){
  ...
  return <div style={{maxWidth:500,margin:"8px auto 0",background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <p style={{color:C.text,fontSize:14,fontWeight:700,margin:0}}>Ajustes</p>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      <Btn onClick={handleExport} color={C.gymColor} style={{width:"100%",justifyContent:"center"}}>Exportar datos</Btn>
...
function AppInner(){
  ...
  return <div style={{fontFamily:"-apple-system,'SF Pro Text','SF Pro Display',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,paddingBottom:72,transition:"background 0.25s,color 0.25s"}}>
    <ToastContainer/>
    {/* Header */}
    <div style={{background:C.bg,borderBottom:`1px solid ${C.border}`,padding:"13px 20px 11px",position:"sticky",top:0,zIndex:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",maxWidth:500,margin:"0 auto"}}>
        <h1 style={{fontSize:17,fontWeight:600,margin:0,letterSpacing:-0.3,color:C.text}}>{titles[screen]}</h1>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <button onClick={()=>setIsDark(d=>!d)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.textSub,padding:"6px 9px",cursor:"pointer",fontSize:15,lineHeight:1,display:"flex",alignItems:"center"}}>
            {isDark?"☀️":"🌙"}
          </button>
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:C.bg,borderTop:`1px solid ${C.border}`,display:"flex",padding:"8px 4px 20px"}}>
      {nav.map(t=>{
        const isActive=screen===t.id;
        return <button key={t.id} onClick={()=>setScreen(t.id)} style={{flex:1,background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",padding:"4px 0"}}>
          {isActive&&<div style={{width:16,height:2,borderRadius:1,background:C.accent,marginBottom:4}}/>}
          {!isActive&&<div style={{width:16,height:2,marginBottom:4}}/>}
          <span style={{fontSize:11,color:isActive?C.text:C.textMuted,fontWeight:isActive?600:400,letterSpacing:-0.2,transition:"color 0.15s"}}>{t.label}</span>
        </button>;
      })}
    </div>
  </div>;
