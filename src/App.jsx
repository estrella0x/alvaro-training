    ...
    <div style={{display:"flex",gap:0,marginBottom:16,background:C.bg,borderRadius:10,padding:3,border:`1px solid ${C.border}`}}>
      {ENT_TABS.map(([id,label])=>(
        <button key={id} onClick={()=>setEntTab(id)}
          style={{flex:1,background:entTab===id?C.card:"transparent",color:entTab===id?C.text:C.textMuted,border:entTab===id?`1px solid ${C.border}`:"1px solid transparent",borderRadius:8,padding:"7px 4px",fontSize:11,fontWeight:entTab===id?600:400,cursor:"pointer",transition:"all 0.12s"}}>
          {label}
        </button>
      ))}
    </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto",paddingBottom:2}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)}
            style={{background:activeTab===t.id?C.accent:C.surface,color:activeTab===t.id?"#fff":C.textMuted,border:`1px solid ${activeTab===t.id?C.accent:C.border}`,borderRadius:8,padding:"8px 14px",fontSize:12,fontWeight:activeTab===t.id?500:400,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,transition:"all 0.15s"}}>
        ...
        {!showAddTab
          ...
          :<div style={{display:"flex",gap:6,flexShrink:0}}>
             ...
             <Btn onClick={addTab} color={C.accent} style={{padding:"8px 10px",fontSize:12}}>✓</Btn>
             <button onClick={()=>setShowAddTab(false)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.textMuted,padding:"8px 10px",cursor:"pointer",fontSize:12}}>✕</button>
           </div>
        }
      </div>
  ...
  return <div>
    ...
    {kcalObj>0&&<Card style={{marginBottom:12}}>
      ...
      <div style={{display:"flex",gap:4,marginBottom:4}}>
        {last7.map((d,i)=><div key={i} style={{flex:1,height:6,borderRadius:4,background:d.kcal?C.accent+"88":C.borderLight}}/>)}
      </div>
      ...
      <Card style={{marginBottom:12}}>
        <div style={{display:"flex",gap:0}}>
          <div style={{flex:1,paddingRight:12,borderRight:`1px solid ${C.border}`}}>
            ...
            <p style={{color:C.text,fontSize:17,fontWeight:700,margin:"0 0 4px",lineHeight:1,letterSpacing:-0.3}}>{totalProt}<span style={{color:C.textMuted,fontSize:11,fontWeight:400}}>/{pt}g</span></p>
          ...
          <div style={{flex:1,paddingLeft:12}}>
            ...
            <p style={{color:C.text,fontSize:17,fontWeight:700,margin:"0 0 4px",lineHeight:1,letterSpacing:-0.3}}>{totalKcal}<span style={{color:C.textMuted,fontSize:11,fontWeight:400}}>/{kcalObj}</span></p>
        ...
        {todayMeals.map((meal)=>{
          ...
          return <div key={meal._idx} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderTop:`1px solid ${C.border}`}}>
            <button onClick={()=>consumeMeal(meal)}
              style={{width:20,height:20,borderRadius:5,flexShrink:0,background:done?C.green:C.surface,border:`2px solid ${done?C.green:C.borderLight}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,fontWeight:700}}>
              {done?"✓":""}
            </button>
      ...
      <Card style={{marginBottom:12}}>
        ...
        {showManual&&<div style={{background:C.surface,borderRadius:8,padding:"10px",marginBottom:8}}>
          <input value={manualName} onChange={e=>setManualName(e.target.value)} placeholder="Nombre (ej: Pollo + arroz)"
            style={{width:"100%",boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"7px 10px",fontSize:12,outline:"none",marginBottom:8}}/>
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            <div style={{flex:1}}>
              <p style={{color:C.textMuted,fontSize:10,margin:"0 0 3px"}}>Proteína (g)</p>
              <input value={manualProt} onChange={e=>setManualProt(e.target.value)} type="number" inputMode="numeric" placeholder="0"
                style={{width:"100%",boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"7px 10px",fontSize:12,outline:"none"}}/>
            </div>
            <div style={{flex:1}}>
              <p style={{color:C.textMuted,fontSize:10,margin:"0 0 3px"}}>Kcal</p>
            ...
            {MEAL_TYPES.map((mt,mi)=>{
              ...
              return <div key={mi} style={{marginBottom:8}}>
                ...
                <input type="text" value={m.desc||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],desc:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                  placeholder="Descripción…" style={{width:"100%",boxSizing:"border-box",background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 8px",fontSize:11,outline:"none",marginBottom:4}}/>
                <div style={{display:"flex",gap:5}}>
                  <input type="number" inputMode="numeric" value={m.prot||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],prot:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                    placeholder="Prot g" style={{flex:1,minWidth:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 6px",fontSize:10,outline:"none"}}/>
                  <input type="number" inputMode="numeric" value={m.kcal||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],kcal:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                    placeholder="Kcal" style={{flex:1,minWidth:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 6px",fontSize:10,outline:"none"}}/>
                </div>
              </div>;
            })}
          </div>}
