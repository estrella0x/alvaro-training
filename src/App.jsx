        ...
        {Object.entries(planos).map(([k,p])=>(
          <div key={k} style={{display:"flex",alignItems:"center",gap:1}}>
            <button onClick={()=>{setPlanoSel(k);setSessionSaved({});setSeriesInputs({});setSessionRPE(null);setSessionNota("");}}
              style={{background:planoSel===k?C.card:C.surface,color:planoSel===k?C.accent:C.textSub,border:`1px solid ${planoSel===k?C.accent:C.border}`,borderRadius:8,padding:"8px 14px",cursor:"pointer",fontSize:12,fontWeight:planoSel===k?600:400,transition:"all 0.15s"}}>
              {k}
            </button>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,overflowX:"auto",paddingBottom:2}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)}
            style={{background:activeTab===t.id?C.card:C.surface,color:activeTab===t.id?C.accent:C.textSub,border:`1px solid ${activeTab===t.id?C.accent:C.border}`,borderRadius:8,padding:"7px 14px",fontSize:11,fontWeight:activeTab===t.id?600:400,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,transition:"all 0.15s"}}>
  ...
  return <div>
    ...
    <Card style={{marginTop:14}}>
      ...
      {[[1.2,"Sedentario"],[1.375,"Ligero — 1-3 días/sem"],[1.55,"Moderado — 3-5 días/sem"],[1.725,"Activo — 6-7 días/sem"],[1.9,"Muy activo — 2x/día"]].map(([val,label])=>(
        <button key={val} onClick={()=>saveProfile({...profile,actividad:val})} style={{display:"block",width:"100%",background:profile.actividad===val?C.card:C.surface,color:profile.actividad===val?C.text:C.textSub,border:`1px solid ${profile.actividad===val?C.accent:C.border}`,borderRadius:8,padding:"8px 12px",cursor:"pointer",fontSize:11,textAlign:"left",fontWeight:profile.actividad===val?600:400,marginBottom:4}}>{profile.actividad===val?"✓ ":""}{label}</button>
      ))}
    </Card>
  </div>;
}
    ...
    {nutTab==="registro"&&<div>
      ...
      <Card style={{marginBottom:12}}>
        ...
        {showManual&&<div style={{background:C.surface,borderRadius:8,padding:"10px",marginBottom:8}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={addManual} style={{flex:1,background:C.accent,border:"none",borderRadius:8,color:"#fff",padding:"8px",cursor:"pointer",fontSize:12,fontWeight:600}}>Añadir</button>
      ...
      <Card>
        ...
        {addingSup
          ?<div style={{marginTop:8,width:"100%",boxSizing:"border-box",background:C.surface,borderRadius:8,padding:"10px",border:`1px solid ${C.border}`}}>
             ...
             <div style={{display:"flex",gap:6}}>
               <button onClick={addSup} style={{flex:1,background:C.accent,border:"none",borderRadius:8,color:"#fff",padding:"7px",cursor:"pointer",fontSize:12,fontWeight:600}}>Añadir</button>
