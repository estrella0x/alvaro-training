...
function buildSessionCache(){
  ...
  tabs.filter(t=>t.type==="cf").forEach(tab=>{
    ...
    const mark=f=>{ if(!f||typeof f!=="string") return;
      if(!byDate[f]) byDate[f]=new Map();
      if(!byDate[f].has(tab.id))
        byDate[f].set(tab.id,{tabId:tab.id,tabName:tab.name,icon:tab.icon,color:tab.color}); };
    Object.entries(data).forEach(([k,hist])=>{
      if(Array.isArray(hist)){ hist.forEach(e=>mark(e&&e.fecha)); }
      else if(k.indexOf("bsess_")===0&&hist&&typeof hist==="object"){ mark(hist.fecha); }
    });
  });
...
function CFPanel({tab,onDeleteTab,autoStart=false,onSessionStarted}){
  ...
  function loadBSessions(){ return Object.entries(data).filter(([k])=>k.startsWith("bsess_")).map(([,v])=>v).filter(v=>v&&typeof v==="object"&&!Array.isArray(v)).sort((a,b)=>(b.id||0)-(a.id||0)); }
  ...
  function bsLastWod(titulo,beforeId){ const t=normalizeName(titulo||""); if(!t) return null;
    for(const s of loadBSessions()){ if(beforeId&&s.id>=beforeId) continue;
      for(const b of safeArr(s.bloques)){ if(b&&b.tipo==="wod"&&normalizeName(b.titulo||"")===t&&(b.resultado||"").trim()) return {resultado:b.resultado,fecha:s.fecha,scoreType:b.scoreType||"",secs:b.secs,enc:(b.scoreType==="amrap")?((b.wRounds||0)*1000+(b.wReps||0)):null}; } }
    return null; }
