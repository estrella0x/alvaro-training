...
const K = {
  ...
  nutProfile:"pg_nup", mealPlan:"pg_mp", shopping:"pg_sh", proteinLog:"pg_pl", recipes:"pg_recipes", weeklyReview:"pg_wrev", weeklyProposal:"pg_wprop", weeklyProposalBackup:"pg_wprop_bak",
};
...
function _v4NutTargets(){
  ...
  const pw=Array.isArray(meds)&&meds.length?safeNum(meds[0]?.peso,65.7):65.7;
  ...
  return { pt:Math.round(pw*2), kcalObj };
}
        ...
        <div key={i} style={{padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
          {editEntry===i?(
            <div style={{display:"flex",gap:8}}>
              ...
              <Btn onClick={()=>{const arr=[...(cargas[histEj]||[])];arr[arr.length-1-i]={...arr[arr.length-1-i],kg:parseFloat(editEntryVal)};saveCargas({...cargas,[histEj]:arr});setEditEntry(null);setEditEntryVal("");}} color={plano.color} style={{padding:"7px 12px",fontSize:12}}>✓</Btn>
              <button onClick={()=>setEditEntry(null)} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:14}}>✕</button>
            </div>
          ):(
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
        {Object.entries(planos).map(([k,p])=>(
          <div key={k} style={{display:"flex",alignItems:"center",gap:1}}>
            <button onClick={()=>{setPlanoSel(k);setSessionSaved({});setSeriesInputs({});setSessionRPE(null);setSessionNota("");}}
              style={{background:planoSel===k?C.card:C.surface,color:planoSel===k?C.text:C.textSub,border:`1px solid ${planoSel===k?C.text:C.border}`,borderRadius:8,padding:"7px 14px",cursor:"pointer",fontSize:12,fontWeight:planoSel===k?600:400,transition:"all 0.15s"}}>
              {k}
            </button>
        ...
        {addingPlano
          ...
          :<button onClick={()=>setAddingPlano(true)} style={{background:C.surface,border:`1px dashed ${C.border}`,borderRadius:8,color:C.textSub,padding:"7px 14px",cursor:"pointer",fontSize:12,fontWeight:400}}>+</button>
        }
      </div>
      ...
      return <div key={ej.id} style={{background:done?C.text+"0d":C.card,border:`1px solid ${done?C.text+"35":C.border}`,borderRadius:12,padding:16,marginBottom:12,transition:"all 0.2s"}}>
        ...
        {!done&&!editMode&&<>
          ...
          <div style={{display:"flex",gap:8,marginBottom:6,alignItems:"center"}}>
            ...
            <button onClick={()=>setTimerSecs({ejId:ej.id,secs:ej.descanso||90})} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.textMuted,padding:"7px 12px",cursor:"pointer",fontSize:12,lineHeight:1}}>⏱</button>
    ...
    {todayAssignments.length===0?(
      ...
      <Card style={{marginBottom:12,background:C.card}}>
        {todayAssignments.map((a,i)=>{
          ...
          return <div key={i} style={{paddingTop:i?12:0,marginTop:i?12:0,borderTop:i?`1px solid ${C.border}`:"none"}}>
            ...
            {!done&&<button onClick={()=>onEmpezar(a.tabId,a.planoKey)} style={{width:"100%",background:C.text,color:C.bg,border:"none",borderRadius:8,padding:"8px",cursor:"pointer",fontSize:13,fontWeight:600,letterSpacing:-0.2}}>Empezar →</button>}
...
function ProgresoSemana(){
  ...
  const v4Generate = () => { persistWeeklyArtifacts(); v4Reload(); pushToast({type:"success", text:"Propuesta semanal generada"}); };
  ...
  const v4ApplyAll = () => setV4Confirm({ msg:"¿Aplicar toda la propuesta (semana, comidas y compra)?", onConfirm:()=>{ setV4Confirm(null); v4RunApply({applyTraining:true,applyNutrition:true,applyShopping:true,replaceShopping:false}, "Propuesta aplicada"); } });
  const v4Apply = v4ApplyAll; // alias de compatibilidad (el botón "Aplicar" anterior → confirmar + aplicar todo)
  const v4ApplyWeek = () => v4RunApply({applyTraining:true,applyNutrition:false,applyShopping:false}, "Semana aplicada");
  const v4ApplyMeals = () => v4RunApply({applyTraining:false,applyNutrition:true,applyShopping:false}, "Comidas aplicadas");
  const v4AddShopping = () => v4RunApply({applyTraining:false,applyNutrition:false,applyShopping:true,replaceShopping:false}, "Compra añadida");
  const v4ReplaceShopping = () => setV4Confirm({ msg:"¿Reemplazar toda la lista de compra por la propuesta?", onConfirm:()=>{ setV4Confirm(null); v4RunApply({applyTraining:false,applyNutrition:false,applyShopping:true,replaceShopping:true}, "Compra reemplazada"); } });
  const v4Undo = () => {
    const r = revertLastWeeklyProposal();
    v4Reload();
    pushToast((r&&r.ok) ? {type:"success", text:"Cambios deshechos"} : {type:"warning", text:"No hay nada que deshacer"});
  };
  ...
  return <div>
    ...
    <Card style={{marginBottom:12}}>
      ...
      {(()=>{
        ...
        if(!rev&&!prop) return <p style={{color:C.textSub,fontSize:12,margin:"0 0 10px",lineHeight:1.4}}>Aún no has generado ninguna propuesta. Pulsa "Generar propuesta".</p>;
        ...
        return <div style={{marginBottom:10}}>
          ...
          {prop&&<p style={{color:C.text,fontSize:12,fontWeight:600,margin:"0 0 6px",lineHeight:1.35}}>{v4ProposalHeadline(prop)}</p>}
          {rev&&<p style={{color:C.textSub,fontSize:11,margin:"0 0 3px"}}>{v4ReviewLine(rev)}</p>}
          {rev&&(()=>{const m=[];if(rev.adherencePct!=null)m.push(`Adherencia ${rev.adherencePct}%`);if(rev.avgRPE!=null&&rev.avgRPE>0)m.push(`RPE ${rev.avgRPE}`);if(rev.proteinDaysHit!=null)m.push(`Proteína ${rev.proteinDaysHit}/7`);if(rev.prsCount!=null)m.push(`PRs ${rev.prsCount}`);if(Array.isArray(rev.alerts)&&rev.alerts.length)m.push(`Alertas ${rev.alerts.length}`);return m.length?<p style={{color:C.textSub,fontSize:11,margin:"0 0 3px"}}>{m.join(" · ")}</p>:null;})()}
          {prop&&<p style={{color:C.textSub,fontSize:11,margin:0}}>Entreno: {v4ActionLabel("training",prop.training&&prop.training.action)} · Nutrición: {v4ActionLabel("nutrition",prop.nutrition&&prop.nutrition.action)} · Compra: {v4ActionLabel("shopping",prop.shopping&&prop.shopping.action)}</p>}
        </div>;
      })()}
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        <Btn onClick={v4Generate} outline style={{padding:"6px 11px",fontSize:11}}>Generar propuesta</Btn>
        <Btn onClick={v4ApplyAll} outline style={{padding:"6px 11px",fontSize:11}}>Aplicar todo</Btn>
        <Btn onClick={v4ApplyWeek} outline style={{padding:"6px 11px",fontSize:11}}>Solo semana</Btn>
        <Btn onClick={v4ApplyMeals} outline style={{padding:"6px 11px",fontSize:11}}>Solo comidas</Btn>
        <Btn onClick={v4AddShopping} outline style={{padding:"6px 11px",fontSize:11}}>Añadir compra</Btn>
        <Btn onClick={v4ReplaceShopping} outline style={{padding:"6px 11px",fontSize:11}}>Reemplazar compra</Btn>
        <Btn onClick={v4Undo} outline style={{padding:"6px 11px",fontSize:11}}>Deshacer</Btn>
      </div>
    </Card>
  ...
  return <div style={{padding:"2px 8px 100px"}}>
      ...
      {(()=>{
        ...
        return <Card style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {recs.items.map((s,ri)=>(
              <button key={s.name} onClick={()=>quickAddRec(s)} style={{flex:"1 1 0",minWidth:120,background:ri===0?C.text+"14":C.surface,border:`1px solid ${ri===0?C.text:C.border}`,borderRadius:8,padding:"7px 9px",cursor:"pointer",textAlign:"left",lineHeight:1.25}}>
                <span style={{color:C.text,fontSize:11,fontWeight:500,display:"block"}}><span style={{color:C.textMuted,fontWeight:600}}>+ </span>{s.name}</span>
    ...
    {nutTab==="plan"&&<div>
      ...
      {(lastTrainedToday||(!lastTrainedToday&&loadWeek()[(new Date().getDay()+6)%7]?.assignments?.length>0))&&(
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"7px 12px",marginBottom:10}}>
                ...
                {isEmptyMeal&&mealSugs.length>0&&<div style={{marginBottom:5}}>
                  ...
                  {mealSugs.map((s,si)=>(
                    <button key={s.name} onClick={()=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],desc:s.name,prot:s.prot,kcal:s.kcal};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                      ...
                      <span style={{color:C.textMuted,fontSize:9,flexShrink:0}}>{s.kcal} · {s.prot}g</span>
                    </button>
                  ))}
                  </div>}
                </div>}
