...
function bsIndex(tipo){ const map={};
  ...
  if(mode==="biblioteca"){
    ...
    if(entry){ const r=libResolveState(entry);
      return <div>
        <BackBtn onClick={()=>{ setLibEntryId(null); }}/>
        ...
        {entry.prerequisito&&<><SectionLabel>Prerequisito</SectionLabel><p style={{color:C.textSub,fontSize:12,margin:"0 0 12px"}}>{entry.prerequisito}</p></>}
        {entry.variantes&&entry.variantes.length>0&&<><SectionLabel>Variantes</SectionLabel><div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>{entry.variantes.map(v=>(<span key={v} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,color:C.textSub,fontSize:11,padding:"4px 9px"}}>{v}</span>))}</div></>}
      </div>;
    }
    return <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      ...
      {libIndex().map(g=>(<div key={g.familia} style={{marginBottom:14}}>
        <SectionLabel>{g.familia}</SectionLabel>
        {g.items.map(e=>{ const r=libResolveState(e); return <button key={e.id} onClick={()=>setLibEntryId(e.id)} style={{display:"block",width:"100%",textAlign:"left",background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 12px",marginBottom:8,cursor:"pointer"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
  ...
  return <div>
    ...
    {tab.id==="skills"&&<button onClick={()=>{ setLibEntryId(null); setMode("biblioteca"); }} style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.textSub,cursor:"pointer",fontSize:11,fontWeight:600,padding:"9px",marginBottom:10}}>Explorar biblioteca</button>}
