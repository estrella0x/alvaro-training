...
function SectionLabel({children,style={}}){
  return <p style={{color:C.textMuted,fontSize:10,letterSpacing:1.2,textTransform:"uppercase",margin:"0 0 8px",fontWeight:500,...style}}>{children}</p>;
}
...
function ProgressBar({pct,color,height=4}){
  return <div style={{background:C.borderLight,borderRadius:height,height,overflow:"hidden"}}>
...
function ConfirmDialog({msg,onConfirm,onCancel}){
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"0 0 20px"}}>
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20,maxWidth:480,width:"calc(100% - 32px)"}}>
      <p style={{color:C.text,fontSize:14,fontWeight:600,margin:"0 0 6px",letterSpacing:-0.3}}>Confirmar acción</p>
  ...
  return <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:12,marginBottom:12}}>
    ...
    <div style={{display:"flex",gap:8,marginBottom:8}}>
      <div style={{flex:1}}>
        ...
        <Input value={minutes} onChange={e=>setMinutes(e.target.value)} placeholder="Ej: 45" style={{fontSize:11}}/>
      </div>
      <div style={{flex:1}}>
...
function CFPanel({tab,onDeleteTab,autoStart=false,onSessionStarted}){
  ...
  if(mode==="detail"&&detailId){
    ...
    return <div>
        ...
        {editingLevels&&<div>
          {niveles.map((n,i)=>(
            <div key={i} style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>
              ...
              <Input type="text" value={n} onChange={e=>{const arr=[...niveles];arr[i]=e.target.value;updateNiveles(detailId,arr);}} style={{flex:1,fontSize:11,padding:"6px 8px"}}/>
          ...
          <div style={{display:"flex",gap:6,marginTop:8}}>
            <Input type="text" value={newLevel} onChange={e=>setNewLevel(e.target.value)} placeholder="Nuevo nivel..." style={{flex:1,fontSize:11,padding:"6px 8px"}}/>
      ...
      {!addPastOpen?(
        <button onClick={()=>setAddPastOpen(true)} style={{background:"none",border:`1px dashed ${C.borderLight}`,borderRadius:12,color:C.textMuted,width:"100%",padding:"10px",cursor:"pointer",fontSize:12,marginTop:10}}>+ Añadir sesión anterior</button>
      ):(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:14,marginTop:10}}>
          ...
          <Input type="text" value={pastNota} onChange={e=>setPastNota(e.target.value)} placeholder="Nota (opcional)" style={{marginBottom:8,fontSize:11}}/>
          <div style={{display:"flex",gap:8}}>
            <Btn onClick={()=>{
        ...
        return <div key={ejId} style={{background:done?color+"12":C.card,border:`1px solid ${done?color+"35":C.border}`,borderRadius:12,padding:14,marginBottom:12,transition:"all 0.2s"}}>
          ...
          {!done&&<>
            {niveles.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
              {niveles.map((n,i)=>(
                <button key={i} onClick={()=>setSessionInputs(s=>({...s,[ejId]:{...s[ejId],nivel:i}}))} style={{background:inp.nivel===i?color:C.surface,color:inp.nivel===i?"#000":C.textSub,border:`1px solid ${inp.nivel===i?color:C.borderLight}`,borderRadius:8,padding:"6px 12px",fontSize:10,cursor:"pointer",fontWeight:inp.nivel===i?700:400}}>{i+1}. {n}</button>
              ))}
            </div>}
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <div style={{flex:1}}>
            ...
            <Input type="text" value={inp.nota||""} onChange={e=>setSessionInputs(s=>({...s,[ejId]:{...s[ejId],nota:e.target.value}}))} placeholder="Nota técnica..." style={{marginBottom:8,fontSize:11}}/>
            <div style={{display:"flex",gap:8}}>
              <Btn onClick={()=>{
...
function GymPanel({initPlanoKey=null,autoStart=false,onSessionStarted}){
  ...
  if(showSummary){
    ...
    return <div>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:14}}>
        ...
        <p style={{color:C.text,fontSize:16,fontWeight:600,margin:"0 0 6px",letterSpacing:-0.2}}>Plano {planoSel} · {plano.nombre}</p>
  ...
  return <div>
        ...
        {Object.entries(planos).map(([k,p])=>(
          <div key={k} style={{display:"flex",alignItems:"center",gap:1}}>
            <button onClick={()=>{setPlanoSel(k);setSessionSaved({});setSeriesInputs({});setSessionRPE(null);setSessionNota("");}}
              style={{background:planoSel===k?C.accent:C.surface,color:planoSel===k?"#fff":C.textSub,border:`1px solid ${planoSel===k?C.accent:C.border}`,borderRadius:8,padding:"8px 14px",cursor:"pointer",fontSize:12,fontWeight:planoSel===k?600:400,transition:"all 0.15s"}}>
              {k}
            </button>
        ...
        {addingPlano
          ?<div style={{display:"flex",gap:4,alignItems:"center"}}>
             <input value={newPlanoName} onChange={e=>setNewPlanoName(e.target.value)} placeholder="D" maxLength={2} autoFocus
               ...
               style={{width:36,background:C.surface,border:`1px solid ${C.accent}`,borderRadius:6,color:C.text,padding:"5px 8px",fontSize:11,outline:"none"}}/>
          ...
          :<button onClick={()=>setAddingPlano(true)} style={{background:"none",border:`1px dashed ${C.border}`,borderRadius:8,color:C.textMuted,padding:"8px 12px",cursor:"pointer",fontSize:16,lineHeight:1}}>+</button>
        }
      </div>
    ...
    {doneCount>0&&<div style={{marginBottom:14}}>
      ...
      <Input type="text" value={sessionNota} onChange={e=>setSessionNota(e.target.value)} placeholder="Nota de sesión: cómo te has sentido, contexto..." style={{fontSize:11,marginTop:6}}/>
    </div>}
    ...
    {safeArr(plano.ejercicios).map((ej,idx)=>{
      ...
      return <div key={ej.id} style={{background:done?C.accent+"12":C.card,border:`1px solid ${done?C.accent+"35":C.border}`,borderRadius:12,padding:16,marginBottom:12,transition:"all 0.2s"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
          <div style={{flex:1}}>
        ...
        {editMode&&<div style={{display:"flex",gap:6,marginBottom:8,borderTop:`1px solid ${C.border}`,paddingTop:8}}>
          <button onClick={()=>moveEj(planoSel,idx,-1)} disabled={idx===0} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.textSub,padding:"6px 12px",cursor:"pointer",fontSize:12,opacity:idx===0?0.3:1}}>↑</button>
          <button onClick={()=>moveEj(planoSel,idx,1)} disabled={idx===plano.ejercicios.length-1} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.textSub,padding:"6px 12px",cursor:"pointer",fontSize:12,opacity:idx===plano.ejercicios.length-1?0.3:1}}>↓</button>
          <button onClick={()=>deleteEj(ej.id)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.textMuted,padding:"6px 12px",cursor:"pointer",fontSize:12,marginLeft:"auto"}}> Eliminar</button>
        </div>}
            ...
            {Array.from({length:ej.series},(_,si)=>{
              ...
              return <div key={si} style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>
                ...
                <input type="number" value={val}
                  ...
                  onBlur={()=>{}}
                  placeholder={lastSeries[si]?`${lastSeries[si]}kg`:"kg"} style={{background:confirmed?C.green+"18":C.surface,border:`1px solid ${confirmed?C.green+"55":C.border}`,borderRadius:8,color:C.text,padding:"6px 8px",fontSize:11,flex:1,boxSizing:"border-box",outline:"none"}}/>
              </div>;
            })}
          </div>
          {/* Timer button inline — RestTimer shown as sticky overlay above */}
          <div style={{display:"flex",gap:8,marginBottom:6,alignItems:"center"}}>
            <Input type="text" value={notes[ej.id]||""} onChange={e=>setNotes(n=>({...n,[ej.id]:e.target.value}))} placeholder="Nota técnica..." style={{flex:1,fontSize:11}}/>
...
function ScreenEntreno({initTab="gym",initPlanoKey=null,autoStart=false,onSessionStarted}){
  ...
  function addTab(){
    ...
    const tab={id,name:newTabName.trim(),icon:"",type:"cf",color:C.accent};
    saveTabs([...tabs,tab]);
  ...
  return <div style={{padding:"2px 2px 100px"}}>
    {confirm&&<ConfirmDialog msg={confirm.msg} onConfirm={confirm.onConfirm} onCancel={()=>setConfirm(null)}/>}
    ...
    <div style={{display:"flex",gap:0,marginBottom:14,background:C.bg,borderRadius:10,padding:3,border:`1px solid ${C.border}`}}>
      {ENT_TABS.map(([id,label])=>(
        <button key={id} onClick={()=>setEntTab(id)}
          style={{flex:1,background:entTab===id?C.card:"transparent",color:entTab===id?C.accent:C.textMuted,border:entTab===id?`1px solid ${C.border}`:"1px solid transparent",borderRadius:8,padding:"7px 4px",fontSize:11,fontWeight:entTab===id?600:400,cursor:"pointer",transition:"all 0.12s",letterSpacing:entTab===id?-0.1:0}}>
          {label}
        </button>
      ))}
    </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,overflowX:"auto",paddingBottom:2}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)}
            style={{background:activeTab===t.id?C.accent:C.surface,color:activeTab===t.id?"#fff":C.textSub,border:`1px solid ${activeTab===t.id?C.accent:C.border}`,borderRadius:8,padding:"7px 14px",fontSize:11,fontWeight:activeTab===t.id?600:400,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,transition:"all 0.15s"}}>
            {t.icon?t.icon+" ":""}{t.name}
          </button>
        ))}
        {!showAddTab
          ?<button onClick={()=>setShowAddTab(true)} style={{background:"none",border:`1.5px dashed ${C.borderLight}`,borderRadius:8,padding:"8px 14px",fontSize:12,color:C.textMuted,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>+ Añadir</button>
          :<div style={{display:"flex",gap:6,flexShrink:0}}>
             <Input value={newTabName} onChange={e=>setNewTabName(e.target.value)} placeholder="Nombre..." style={{width:120}}/>
             <Btn onClick={addTab} color={C.accent} style={{padding:"8px 12px",fontSize:12}}>✓</Btn>
...
function MedidasPanel(){
  ...
  if(mode==="add") return <div>
    <BackBtn onClick={()=>setMode("list")}/>
    <p style={{color:C.text,fontSize:16,fontWeight:700,margin:"0 0 16px"}}>Nueva evaluación</p>
    ...
    <Btn onClick={handleAdd} color={C.accent} style={{width:"100%",padding:"12px",marginTop:4}}>Guardar evaluación</Btn>
  </div>;

  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <SectionLabel>Evaluaciones</SectionLabel>
      <button onClick={()=>setMode("add")} style={{background:"none",border:"none",color:C.accent,cursor:"pointer",fontSize:13,fontWeight:500,padding:0}}>+ Nueva</button>
    </div>
      ...
      return <div key={i} style={{marginBottom:8}}>
        <div style={{display:"flex",alignItems:"stretch",gap:6}}>
          <button onClick={()=>setOpenIdx(isOpen?null:i)}
            style={{flex:1,background:isOpen?C.card:C.surface,border:`1px solid ${isOpen?C.accent:C.border}`,borderRadius:8,padding:"10px 14px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",textAlign:"left"}}>
            <div>
  ...
  return <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"12px 14px",marginBottom:12}}>
    <div style={{display:"flex",gap:5,marginBottom:10}}>
      {labels.map((label,i)=>{
        ...
        return <button key={n} onClick={()=>onSave({...sleepData,rating:active?0:n})}
          style={{flex:1,minWidth:0,background:active?C.accent:C.bg,border:`1px solid ${active?C.accent:C.border}`,borderRadius:8,padding:"8px 2px",cursor:"pointer"}}>
    ...
    <div style={{display:"flex",gap:8,alignItems:"center"}}>
      ...
      <input
        ...
        placeholder="7.5"
        inputMode="decimal"
        style={{flex:1,minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,padding:"6px 8px",fontSize:11,outline:"none",WebkitAppearance:"none"}}
      />
...
function NutricionHoy({onNavigate,ydProtPct=100}){
  ...
  return <Card style={{marginBottom:14}}>
    ...
    <ProgressBar pct={Math.min(100,Math.round((hydration/wtgt)*100))} color={watDone?C.green:C.blue} height={4}/>
...
function ScreenHoyMerged({onEmpezar,onGoToNutricion,onGoToSemana,onGoToFisico,onGoToComidas}){
  ...
  return <div style={{padding:"2px 2px 100px"}}>
    ...
    {todayAssignments.length===0?(
      <Card style={{marginBottom:12,background:C.card}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
          <div style={{width:40,height:40,borderRadius:12,background:C.blue+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:400,color:C.textMuted}}>—</div>
          <div>
            <p style={{color:C.text,fontSize:14,fontWeight:600,margin:0}}>Día de descanso</p>
      ...
      <div style={{marginBottom:14}}>
        {todayAssignments.map((a,i)=>{
          ...
          return <div key={i} style={{background:C.card,border:`1px solid ${done?C.accent+"44":C.border}`,borderRadius:12,padding:18,marginBottom:i<todayAssignments.length-1?10:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:done?6:12}}>
              <div>
                ...
                <p style={{color:C.text,fontSize:16,fontWeight:700,margin:0,letterSpacing:-0.3}}>{label}</p>
              </div>
    ...
    <div style={{display:"flex",gap:6,marginBottom:showSleep?8:14}}>
      ...
      {(()=>{const d=dayState[0];return <button onClick={()=>setShowSleep(s=>!s)} style={{flex:1,background:showSleep?C.card:C.surface,border:`1px solid ${showSleep?C.accent:C.border}`,borderRadius:8,padding:"8px 6px",textAlign:"center",cursor:"pointer",position:"relative"}}>
        <p style={{color:C.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:0,margin:"0 0 4px",fontWeight:500}}>{d.label}</p>
    ...
    {(()=>{
      ...
      return objs.map(obj=>{
        ...
        return <button key={obj.id} onClick={onGoToFisico} style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 14px",cursor:"pointer",textAlign:"left",marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:pct!==null?8:0}}>
            <div>
              <p style={{color:C.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:1.2,margin:"0 0 2px",fontWeight:500}}>{obj.name||"Objetivo"}</p>
...
function ProgresoSemana(){
  ...
  return <div>
    ...
    {(()=>{
      ...
      return <Card style={{marginBottom:14}}>
        <p style={{color:C.text,fontSize:13,fontWeight:600,margin:"0 0 12px",letterSpacing:-0.2}}>Esta semana</p>
    ...
    {kcalObj>0&&<Card style={{marginBottom:12}}>
      ...
      <div style={{display:"flex",gap:4,marginBottom:4}}>
        {last7.map((d,i)=><div key={i} style={{flex:1,height:6,borderRadius:6,background:d.kcal?C.accent+"88":C.borderLight}}/>)}
      </div>
  ...
  return <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>
        {[["Proteínas",proteinTarget+"g"],["Carbohidratos",carbTarget+"g"],["Grasas",fatTarget+"g"]].map(([l,v])=>(
          <div key={l} style={{background:C.surface,borderRadius:8,padding:"9px 6px",textAlign:"center"}}>
            <p style={{color:C.textMuted,fontSize:11,margin:"0 0 3px"}}>{l}</p>
    ...
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <Btn onClick={()=>saveObjectives([...objectives,{id:"o"+Date.now(),name:"Nuevo objetivo",start:"",current:"",target:"",unit:""}])} color={C.accent} style={{padding:"6px 12px",fontSize:11}}>+ Añadir</Btn>
      </div>
      {objectives.map((obj,i)=>(
        <Card key={obj.id} style={{marginBottom:8}}>
          <div style={{display:"flex",gap:6,marginBottom:6}}>
            <Input type="text" value={obj.name} onChange={e=>{const upd=[...objectives];upd[i]={...upd[i],name:e.target.value};saveObjectives(upd);}} placeholder="Objetivo" style={{flex:2,fontSize:11}}/>
          ...
          <div style={{display:"flex",gap:6}}>
            <div style={{flex:1}}><p style={{color:C.textMuted,fontSize:11,margin:"0 0 3px"}}>Actual</p><Input value={obj.current} onChange={e=>{const upd=[...objectives];upd[i]={...upd[i],current:e.target.value};saveObjectives(upd);}} style={{fontSize:11}}/></div>
            <div style={{flex:1}}><p style={{color:C.textMuted,fontSize:11,margin:"0 0 3px"}}>Meta</p><Input type="text" value={obj.target} onChange={e=>{const upd=[...objectives];upd[i]={...upd[i],target:e.target.value};saveObjectives(upd);}} style={{fontSize:11}}/></div>
            <div style={{flex:"0 0 55px"}}><p style={{color:C.textMuted,fontSize:11,margin:"0 0 3px"}}>Unidad</p><Input type="text" value={obj.unit} onChange={e=>{const upd=[...objectives];upd[i]={...upd[i],unit:e.target.value};saveObjectives(upd);}} style={{fontSize:11}}/></div>
          </div>
        </Card>
      ))}
    </div>
    ...
    <Card style={{marginTop:14}}>
      ...
      {[[1.2,"Sedentario"],[1.375,"Ligero — 1-3 días/sem"],[1.55,"Moderado — 3-5 días/sem"],[1.725,"Activo — 6-7 días/sem"],[1.9,"Muy activo — 2x/día"]].map(([val,label])=>(
        <button key={val} onClick={()=>saveProfile({...profile,actividad:val})} style={{display:"block",width:"100%",background:profile.actividad===val?"#0a7aff"+"22":C.surface,color:profile.actividad===val?"#0a7aff":C.textSub,border:`1px solid ${profile.actividad===val?"#0a7aff"+"66":C.border}`,borderRadius:8,padding:"8px 12px",cursor:"pointer",fontSize:11,textAlign:"left",fontWeight:profile.actividad===val?700:400,marginBottom:4}}>{profile.actividad===val?"✓ ":""}{label}</button>
      ))}
    </Card>
  </div>;
}
...
function ProgresoPlantificar(){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
      ...
      {templates.length>0&&<button onClick={()=>setShowTpl(s=>!s)} style={{background:showTpl?C.accent+"22":C.surface,border:`1px solid ${showTpl?C.accent:C.border}`,borderRadius:8,color:showTpl?C.accent:C.textSub,padding:"6px 12px",cursor:"pointer",fontSize:11,fontWeight:500}}> {templates.length}</button>}
    </div>
    {showTpl&&<div style={{background:C.surface,borderRadius:12,padding:12,marginBottom:14}}>
      ...
      {templates.map(tpl=>(
        <div key={tpl.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>handleApplyTemplate(tpl)} style={{background:C.accent,color:"#000",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:11,fontWeight:700}}>Aplicar</button>
...
function ShoppingAddForm({onAdd,color}){
  ...
  return <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:14,marginTop:8}}>
    <div style={{display:"flex",gap:8,marginBottom:8}}>
      <Input type="text" value={name} onChange={e=>setName(e.target.value)} placeholder="Nombre del producto" style={{flex:2,fontSize:11}} onEnter={()=>{if(name.trim()){onAdd({id:"s_"+Date.now(),name:name.trim(),cat,done:false});setName("");setOpen(false);}}}/>
    </div>
    <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>
      {CAT_ORDER.map(c=>(
        <button key={c} onClick={()=>setCat(c)} style={{background:cat===c?color:C.surface,color:cat===c?"#000":C.textSub,border:`1px solid ${cat===c?color:C.border}`,borderRadius:8,padding:"6px 12px",fontSize:10,cursor:"pointer",fontWeight:cat===c?700:400}}>{CAT_LABELS[c]||c}</button>
      ))}
    </div>
    <div style={{display:"flex",gap:8}}>
...
function CustomProteinAdd({onAdd,color,onSaveFood}){
  ...
  const [open,setOpen]=useState(false);
  if(!open) return <button onClick={()=>setOpen(true)} style={{background:"none",border:`1px dashed ${color}44`,borderRadius:12,color,width:"100%",padding:"9px",cursor:"pointer",fontSize:12,marginBottom:12}}>+ Añadir alimento personalizado</button>;
  return <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:14,marginBottom:12}}>
    <div style={{display:"flex",gap:8,marginBottom:8}}>
      <Input type="text" value={name} onChange={e=>setName(e.target.value)} placeholder="Nombre" style={{flex:2,fontSize:11}}/>
      <Input value={prot} onChange={e=>setProt(e.target.value)} placeholder="g prot" style={{flex:1,fontSize:11}}/>
    </div>
    <div style={{display:"flex",gap:8}}>
  ...
  return <div style={{padding:"2px 2px 100px"}}>
    ...
    {nutTab==="registro"&&<div>
        ...
        <div style={{display:"flex",gap:0}}>
          <div style={{flex:1,paddingRight:12,borderRight:`1px solid ${C.border}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:4}}>
              <p style={{color:C.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:1,margin:0}}>Proteína</p>
          ...
          <div style={{flex:1,paddingLeft:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:4}}>
              <p style={{color:C.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:1,margin:0}}>Kcal</p>
            ...
            <ProgressBar pct={Math.min(100,Math.round((totalKcal/kcalObj)*100))} color={totalKcal>=kcalObj?C.green:C.accent} height={4}/>
          </div>
        </div>
      </Card>
      ...
      {todayMeals.length>0&&<Card style={{marginBottom:12}}>
        <p style={{color:C.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:1.2,margin:"0 0 8px",fontWeight:500}}>Plan de hoy</p>
        {todayMeals.map((meal)=>{
          const id=`${todayDow}_${meal._idx}`;
          const done=consumedIds.has(id);
          const mt=MEAL_TYPES[meal._idx]||meal.t||"";
          return <div key={meal._idx} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderTop:`1px solid ${C.border}`}}>
      ...
      <Card style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <p style={{color:C.text,fontSize:12,fontWeight:600,margin:0}}>Registro manual</p>
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>setShowManual(s=>!s)}
              style={{background:"none",border:`1px solid ${showManual?C.accent:C.border}`,color:showManual?C.accent:C.textSub,borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:10,fontWeight:500}}>+ Añadir</button>
          </div>
        </div>
        ...
        {showManual&&<div style={{background:C.surface,borderRadius:8,padding:"10px",marginBottom:8}}>
          <input value={manualName} onChange={e=>setManualName(e.target.value)} placeholder="Nombre (ej: Pollo + arroz)"
            style={{width:"100%",boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"8px 12px",fontSize:12,outline:"none",marginBottom:8}}/>
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            <div style={{flex:1}}>
              ...
              <input value={manualProt} onChange={e=>setManualProt(e.target.value)} type="number" inputMode="numeric" placeholder="0"
                style={{width:"100%",boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"6px 8px",fontSize:11,outline:"none"}}/>
            </div>
            <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={addManual} style={{flex:1,background:C.accent,border:"none",borderRadius:6,color:"#fff",padding:"8px",cursor:"pointer",fontSize:12,fontWeight:600}}>Añadir</button>
      ...
      <Card style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:6}}>
          <p style={{color:C.text,fontSize:12,fontWeight:600,margin:0}}>Agua{isWOD?" · WOD":""}</p>
        ...
        {addingSup
          ?<div style={{marginTop:8,width:"100%",boxSizing:"border-box",background:C.surface,borderRadius:8,padding:"10px",border:`1px solid ${C.border}`}}>
             <div style={{display:"flex",gap:6,marginBottom:8}}>
               <input value={newSupName} onChange={e=>setNewSupName(e.target.value)} placeholder="Nombre" onKeyDown={e=>{if(e.key==="Enter")addSup();}}
                 style={{flex:1,minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 8px",fontSize:11,outline:"none"}}/>
               <input value={newSupDose} onChange={e=>setNewSupDose(e.target.value)} placeholder="Dosis"
                 style={{flex:"0 0 66px",minWidth:0,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 8px",fontSize:11,outline:"none"}}/>
             </div>
             <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:newSupIsProtein?8:10}}>
               ...
               <button onClick={()=>setNewSupIsProtein(v=>!v)}
                 style={{background:newSupIsProtein?C.accent:C.bg,border:`1px solid ${newSupIsProtein?C.accent:C.border}`,borderRadius:6,color:newSupIsProtein?"#fff":C.textMuted,padding:"4px 12px",cursor:"pointer",fontSize:12,fontWeight:500}}>
             ...
             {newSupIsProtein&&<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
               ...
               <input value={newSupProtGrams} onChange={e=>setNewSupProtGrams(e.target.value)} placeholder="25" type="number" inputMode="numeric"
                 style={{width:60,boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 8px",fontSize:11,outline:"none"}}/>
             </div>}
             <div style={{display:"flex",gap:6}}>
               <button onClick={addSup} style={{flex:1,background:C.accent,border:"none",borderRadius:6,color:"#fff",padding:"7px",cursor:"pointer",fontSize:12,fontWeight:600}}>Añadir</button>
          ...
          :<button onClick={()=>setAddingSup(true)}
             style={{marginTop:6,background:"none",border:`1px dashed ${C.border}`,borderRadius:6,color:C.textMuted,padding:"6px 12px",cursor:"pointer",fontSize:10,width:"100%"}}>+ Añadir suplemento</button>
        }
      </Card>
    </div>}
      ...
      {orderedDays.map((di)=>{
        ...
        return <div key={di} style={{marginBottom:6}}>
          <button onClick={()=>setExpandedDays(p=>({...p,[di]:!isExpanded}))}
            style={{width:"100%",background:isToday?C.card:C.surface,border:`1px solid ${isToday?nutColor+"44":C.border}`,borderRadius:8,padding:"9px 12px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",textAlign:"left"}}>
            <p style={{color:isToday?nutColor:C.text,fontSize:12,fontWeight:isToday?600:400,margin:0}}>{dayNames[di]}{isToday&&<span style={{color:nutColor,fontSize:10,fontWeight:700,marginLeft:6}}>HOY</span>}</p>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          {isExpanded&&<div style={{background:C.card,border:`1px solid ${C.border}`,borderTopWidth:0,borderRadius:"0 0 8px 8px",padding:"8px 12px"}}>
            {MEAL_TYPES.map((mt,mi)=>{
              ...
              return <div key={mi} style={{marginBottom:8}}>
                <p style={{color:C.textMuted,fontSize:10,margin:"0 0 3px",textTransform:"uppercase",letterSpacing:1}}>{mt}</p>
                <input type="text" value={m.desc||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],desc:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                  placeholder="Descripción…" style={{width:"100%",boxSizing:"border-box",background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"5px 7px",fontSize:11,outline:"none",marginBottom:4}}/>
                <div style={{display:"flex",gap:5}}>
                  <input type="number" inputMode="numeric" value={m.prot||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],prot:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                    placeholder="Prot g" style={{flex:1,minWidth:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"4px 6px",fontSize:11,outline:"none"}}/>
                  <input type="number" inputMode="numeric" value={m.kcal||""} onChange={e=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],kcal:e.target.value};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                    placeholder="Kcal" style={{flex:1,minWidth:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,padding:"4px 6px",fontSize:11,outline:"none"}}/>
                </div>
              </div>;
            })}
          </div>}
    ...
    {/* ── COMPRA ── */}
    {nutTab==="compra"&&<div>
      <div style={{background:C.surface,borderRadius:8,padding:"8px 12px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            {items.map(item=>(
              <div key={item.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
                ...
                <input value={item.name} onChange={e=>saveShopping(shopping.map(s=>s.id===item.id?{...s,name:e.target.value}:s))}
                  style={{flex:1,background:"transparent",border:"none",color:item.done?C.textMuted:C.text,fontSize:11,padding:0,outline:"none",textDecoration:item.done?"line-through":"none",fontFamily:"inherit"}}/>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      ...
      {mode==="paste"&&<>
        <textarea value={jsonText} onChange={e=>setJsonText(e.target.value)} placeholder='Pega tu JSON aquí...' rows={5}
          style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,color:C.text,padding:10,fontSize:12,fontFamily:"monospace",resize:"vertical",width:"100%",boxSizing:"border-box"}}/>
...
function AppInner(){
  ...
  useEffect(()=>{
    ...
    s.textContent="input,textarea,select{font-size:max(16px,1em)!important;} input:focus,textarea:focus{font-size:max(16px,1em)!important;}";
    ...
    return()=>{document.head.removeChild(s);document.removeEventListener("touchmove",noZoom);};
  },[]);
  useEffect(()=>{
  ...
  return <div style={{fontFamily:"-apple-system,'SF Pro Text','SF Pro Display',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,touchAction:"pan-x pan-y",paddingBottom:"max(72px,calc(52px + env(safe-area-inset-bottom,20px)))",transition:"background 0.25s,color 0.25s"}}>
    <ToastContainer/>
    {/* Header */}
    <div style={{background:C.bg,borderBottom:`1px solid ${C.border}`,padding:"13px 20px 11px",paddingTop:"max(13px,env(safe-area-inset-top,13px))",position:"sticky",top:0,zIndex:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",maxWidth:500,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <button onClick={()=>setIsDark(d=>{const next=!d;localStorage.setItem("pg_theme",next?"dark":"light");return next;})} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.textSub,padding:"6px 9px",cursor:"pointer",fontSize:14,lineHeight:1,display:"flex",alignItems:"center"}}>
            {isDark?"☀️":"🌙"}
          </button>
          <button onClick={()=>setShowBackup(b=>!b)} style={{background:showBackup?C.surface:"none",border:`1px solid ${showBackup?C.border:"transparent"}`,borderRadius:8,color:showBackup?C.text:C.textSub,padding:"6px 9px",cursor:"pointer",fontSize:14,lineHeight:1,display:"flex",alignItems:"center"}}>⚙️</button>
        </div>
      </div>
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:C.bg,borderTop:`1px solid ${C.border}`,display:"flex",padding:`8px 4px max(20px,env(safe-area-inset-bottom,20px))`,WebkitBackdropFilter:"blur(0)",transform:"translateZ(0)"}}>
      {nav.map(t=>{
        ...
        return <button key={t.id} onClick={()=>{setScreen(t.id);window.scrollTo({top:0,behavior:"instant"});}} style={{flex:1,background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",padding:"4px 0"}}>
          <span style={{fontSize:11,color:isActive?C.accent:C.textMuted,fontWeight:isActive?600:400,transition:"color 0.15s"}}>{t.label}</span>
          <div style={{width:4,height:4,borderRadius:"50%",background:isActive?C.accent:"transparent",marginTop:3,transition:"background 0.2s"}}/>
        </button>;
      })}
    </div>
  </div>;
