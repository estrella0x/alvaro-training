    ...
    {plano.ejercicios.map((ej,idx)=>{
      ...
      return <div key={ej.id} style={{background:done?C.gymColor+"12":C.card,border:`1px solid ${done?C.gymColor+"35":C.border}`,borderRadius:16,padding:14,marginBottom:10,transition:"all 0.2s"}}>
        ...
        {!done&&!editMode&&<>
          ...
          {/* Rest timer — one per exercise block */}
          {timerSecs&&timerSecs.ejId===ej.id&&<RestTimer seconds={timerSecs.secs} onClose={()=>setTimerSecs(null)}/>}
  ...
  return <Card style={{marginBottom:14}}>
    ...
    {sleepData.rating>0&&<>
      <div style={{display:"flex",gap:8,alignItems:"center",marginTop:4,marginBottom:8}}>
        <p style={{color:C.textMuted,fontSize:12,margin:0}}>Horas:</p>
    ...
    {sleepData.rating>0&&sleepData.rating<=2&&<p style={{color:C.red,fontSize:11,margin:"8px 0 0"}}>⚠️ Sueño deficiente — considera reducir la intensidad hoy.</p>}
    {sleepData.rating>=4&&(()=>{
      ...
      if(todayProt>=Math.round(pw*2)*0.8) return <p style={{color:C.textMuted,fontSize:10,margin:"6px 0 0",fontStyle:"italic"}}>💡 Proteína antes de dormir: 30-40g caseína mejora síntesis nocturna.</p>;
      return <p style={{color:C.green,fontSize:11,margin:"8px 0 0"}}>✅ Buen descanso — condiciones óptimas para crecer.</p>;
    })()}
  </Card>;
}
...
function NutricionHoy(){
  ...
  const last7=useMemo(()=>[...Array(7)].map((_,i)=>{
    ...
    return (load(K.proteinLog)||{})[ds]?.reduce((a,e)=>a+(e.prot||0),0)||0 >= pt;
  }).reverse(),[]);
  const daysHit=last7.filter(Boolean).length;

  return <Card style={{marginBottom:14}}>
    {/* Header with weekly consistency */}
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        <p style={{color:C.text,fontSize:14,fontWeight:700,margin:0}}>🥗 Nutrición</p>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          {isTrainingToday&&<Tag color={C.blue}>+150 kcal</Tag>}
          <Tag color="#f97316">{kcalObj} kcal</Tag>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <div style={{display:"flex",gap:3}}>{last7.map((hit,i)=><div key={i} style={{width:8,height:8,borderRadius:"50%",background:hit?C.green:C.borderLight}}/>)}</div>
        <p style={{color:daysHit>=5?C.green:daysHit>=3?C.orange:C.red,fontSize:10,fontWeight:600,margin:0}}>{daysHit}/7 días con proteína</p>
      </div>
    </div>
    ...
    {showProteinAdd&&<div style={{background:C.surface,borderRadius:12,padding:12,marginBottom:10,marginTop:-4}}>
      ...
      {proteinLog.slice().reverse().slice(0,4).map((e,i)=>(
        <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderTop:`1px solid ${C.border}`}}>
          ...
          <div style={{display:"flex",gap:8}}><p style={{color:nutColor,fontSize:11,fontWeight:700,margin:0}}>+{e.prot}g</p>
            <button onClick={()=>{const upd=[...proteinLog];upd.splice(proteinLog.length-1-i,1);saveProteinLog(upd);}} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:11}}>✕</button></div>
        </div>
      ))}
    </div>}

    {(()=>{const isWOD=loadWeek()[(new Date().getDay()+6)%7]?.assignments?.some(a=>a.tabId==="wod");const wtgt=isWOD?Math.round((ht+0.75)*10)/10:ht;
      return <TrackerRow icon="💧" label={isWOD?"Agua (WOD)":"Agua"} cur={hydration} tgt={wtgt} unit="L" color={C.blue} onReset={()=>saveHydration(0)}
    ...
    {(()=>{
      ...
      return <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderTop:`1px solid ${C.border}`}}>
        <button onClick={()=>{const upd={...reloads};upd[today()]=tr==="recarga"?"libre":tr==="libre"?null:"recarga";save("pg_reloads",upd);}} style={{background:tr?C.yellow+"22":C.surface,border:`1.5px solid ${tr?C.yellow:C.border}`,borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:11,color:tr?C.yellow:C.textMuted,fontWeight:tr?700:400}}>
          {tr==="recarga"?"🔄 Recarga":tr==="libre"?"🍕 Comida libre":"+ Marcar recarga/libre"}
        </button>
    ...
    <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:8}}>
      ...
      <p style={{color:C.textMuted,fontSize:10,margin:"7px 0 0",fontStyle:"italic"}}>💡 Creatina post-entreno con carbos mejora absorción ~60%</p>
    </div>
  </Card>;
}
    ...
    {nutTab==="comidas"&&<div>
      {!lastTrainedToday&&loadWeek()[(new Date().getDay()+6)%7]?.assignments?.length>0&&(
        <div style={{background:C.blue+"15",border:`1px solid ${C.blue}44`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",gap:10}}>
          ...
          <div><p style={{color:C.blue,fontSize:13,fontWeight:700,margin:"0 0 2px"}}>Hoy toca entrenar — pre-entreno</p>
          <p style={{color:C.textSub,fontSize:12,margin:0}}>1-2h antes: <strong>40-60g carbos + 20g proteína</strong></p></div>
        </div>
      )}
      {lastTrainedToday&&<div style={{background:C.green+"15",border:`1px solid ${C.green}44`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",gap:10}}>
        <span style={{fontSize:18}}>💪</span>
        <div><p style={{color:C.green,fontSize:13,fontWeight:700,margin:"0 0 2px"}}>Entrenaste hoy — post-entreno</p>
        <p style={{color:C.textSub,fontSize:12,margin:0}}>En 60 min: <strong>30-40g proteína + 60-80g carbos</strong></p></div>
      </div>}
...
// ── APP ───────────────────────────────────────────────────────────────────────
// ── SCREEN HOY (merged: orientation + daily tracking) ────────────────────────
function ScreenHoyMerged({onEmpezar,onGoToNutricion}){
  ...
  // Cross-fatigue: yesterday's muscles vs today's
  const crossFatigue=(()=>{
  ...
  return <div>
    ...
    {todayAssignments.length===0?(
      <Card style={{marginBottom:14,background:`linear-gradient(135deg,${C.surface},${C.card})`}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={onGoToNutricion} style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.textSub,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:600}}>🥗 Revisar nutrición</button>
        </div>
      </Card>
    ):(
      <div style={{marginBottom:14}}>
    ...
    <div style={{display:"flex",gap:8,marginBottom:14}}>
      {[
        {label:"Esta semana",val:daysThisWeek,unit:"días",color:C.gymColor},
        {label:"Racha",val:streak>0?streak:"—",unit:"sem",color:C.yellow},
        {label:"Sueño",val:sleepToday.rating>0?["😴","💤","😐","😊","🌟"][sleepToday.rating-1]:"—",unit:sleepToday.hours?sleepToday.hours+"h":"",color:sleepToday.rating>=4?C.green:sleepToday.rating>=3?C.yellow:sleepToday.rating>0?C.red:C.textMuted},
      ].map(({label,val,unit,color})=>(
        <Card key={label} style={{flex:1,padding:12,textAlign:"center"}}>
          <p style={{color:C.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:1,margin:"0 0 3px"}}>{label}</p>
          <p style={{color,fontSize:20,fontWeight:800,margin:"0 0 2px",lineHeight:1}}>{val}</p>
    ...
    {showDeload&&<div style={{background:C.orange+"18",border:`1px solid ${C.orange}44`,borderRadius:14,padding:"12px 16px",marginBottom:12,display:"flex",gap:10}}>
      <span>⚠️</span><div><p style={{color:C.orange,fontSize:13,fontWeight:700,margin:"0 0 2px"}}>Deload — {weeksSinceFirst} semanas</p><p style={{color:C.textSub,fontSize:12,margin:0}}>Reduce a 2 series y el 60-70% del peso.</p></div>
    </div>}
    {showFatigue&&<div style={{background:C.red+"14",border:`1px solid ${C.red}44`,borderRadius:14,padding:"12px 16px",marginBottom:12,display:"flex",gap:10}}>
      <span>🔴</span><div><p style={{color:C.red,fontSize:13,fontWeight:700,margin:"0 0 2px"}}>Fatiga alta — RPE {avgRPE.toFixed(1)}/10</p><p style={{color:C.textSub,fontSize:12,margin:0}}>Considera bajar la intensidad o descansar.</p></div>
    </div>}
    {!showDeload&&!showFatigue&&stalledEjs.length>0&&<div style={{background:C.red+"14",border:`1px solid ${C.red}44`,borderRadius:14,padding:"12px 16px",marginBottom:12,display:"flex",gap:10}}>
      <span>⚡</span><div><p style={{color:C.red,fontSize:13,fontWeight:700,margin:"0 0 2px"}}>Estancamiento — {stalledEjs[0]?.nombre}</p><p style={{color:C.textSub,fontSize:12,margin:0}}>Sin progreso en 3 sesiones. Cambia el estímulo.</p></div>
    </div>}
    {crossFatigue&&<div style={{background:C.orange+"14",border:`1px solid ${C.orange}44`,borderRadius:14,padding:"10px 14px",marginBottom:12,display:"flex",gap:8}}>
      <span>😩</span><p style={{color:C.orange,fontSize:12,margin:0}}>{crossFatigue.join(", ")} entrenados ayer — considera ajustar intensidad</p>
    </div>}

    {/* YESTERDAY */}
    {ydSessions.length>0&&<Card style={{marginBottom:12}}>
