...
function normSets(arr){ return safeArr(arr).map(s=>{ if(s&&typeof s==="object"){ const kg=safeNum(s.kg,0); const r=parseInt(s.reps); return {kg,reps:isNaN(r)?null:r,type:s.type||"normal",rpe:(s.rpe!=null&&s.rpe!=="")?Number(s.rpe):null}; } const kg=parseFloat(s); return {kg:isNaN(kg)?0:kg,reps:null,type:"normal",rpe:null}; }).filter(s=>s.kg>0); }
...
function GymPanel({initPlanoKey=null,autoStart=false,onSessionStarted}){
  ...
  const [chartMetric,setChartMetricRaw]=useState(()=>{ const v=load("pg_chartpref"); return (v&&["peso","1rm","vol","reps"].includes(v.metric))?v.metric:"peso"; }); // métrica activa (persistida global)
  const [chartRange,setChartRangeRaw]=useState(()=>{ const v=load("pg_chartpref"); return (v&&["30d","3m","1y","all"].includes(v.range))?v.range:"all"; }); // rango temporal (persistido global)
  ...
  const setChartMetric=(v)=>{ setChartMetricRaw(v); try{ save("pg_chartpref",{metric:v,range:chartRange,agg:chartAgg}); }catch(e){} };
  const setChartRange=(v)=>{ setChartRangeRaw(v); try{ save("pg_chartpref",{metric:chartMetric,range:v,agg:chartAgg}); }catch(e){} };
  const setChartAgg=(v)=>{ setChartAggRaw(v); try{ save("pg_chartpref",{metric:chartMetric,range:chartRange,agg:v}); }catch(e){} };
  const [chartMA,setChartMA]=useState(false); // media móvil (solo vista, Peso/1RM)
  ...
  function handleSaveEj(ejId,descanso,numSeries){
    ...
    const sets=arr.map(o=>{ const kg=pkg(o&&o.kg); const r=parseInt(o&&o.reps); const rp=o&&o.rpe; return {kg:isNaN(kg)?0:kg,reps:isNaN(r)?null:r,type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.kg>0);
  ...
  if(histEj){
    ...
    const _stats=(()=>{ let heaviest=0,oneRM=0,bestSetVol=0,bestSessVol=0,mostReps=0; safeArr(cargas[histEj]).forEach(e=>{ const sets=normSets(e.series).filter(s=>s.type!=="warmup"&&s.kg>0); let sv=0; sets.forEach(s=>{ if(s.kg>heaviest)heaviest=s.kg; if(s.reps!=null&&s.reps>0){ const vol=s.kg*s.reps; if(vol>bestSetVol)bestSetVol=vol; sv+=vol; if(s.reps>mostReps)mostReps=s.reps; if(s.reps<=12){ const e1=s.kg*(1+s.reps/30); if(e1>oneRM)oneRM=e1; } } }); if(sv>bestSessVol)bestSessVol=sv; }); return {heaviest:Math.round(heaviest*100)/100,oneRM:Math.round(oneRM*2)/2,bestSetVol:Math.round(bestSetVol),bestSessVol:Math.round(bestSessVol),mostReps,has:heaviest>0}; })();
    const _sessionMetric=(e,metric)=>{ const sets=normSets(e.series).filter(s=>s.type!=="warmup"&&s.kg>0); if(metric==="1rm"){ let m=0; sets.forEach(s=>{ if(s.reps!=null&&s.reps>=1&&s.reps<=12){ const e1=s.kg*(1+s.reps/30); if(e1>m)m=e1; } }); return m>0?Math.round(m*2)/2:null; } if(metric==="vol"){ let v=0; sets.forEach(s=>{ if(s.reps!=null&&s.reps>0) v+=s.kg*s.reps; }); return v>0?Math.round(v):null; } if(metric==="reps"){ let m=0; sets.forEach(s=>{ if(s.reps!=null&&s.reps>m)m=s.reps; }); return m>0?m:null; } const w=Math.max(0,...sets.map(s=>s.kg)); return w>0?w:null; };
    const _mMeta={peso:["Peso top","kg"],"1rm":["1RM estimado","kg"],vol:["Volumen","kg"],reps:["Reps top","reps"]};
    ...
    const _metricData=safeArr(cargas[histEj]).filter(h=>_inRange(h.fecha)).map(h=>({val:_sessionMetric(h,chartMetric),fecha:h.fecha})).filter(d=>d.val!=null);
    ...
    const _chartSeries=(()=>{ if(chartAgg==="sesion") return _metricData; const isSum=chartMetric==="vol"; const m=new Map(); _metricData.forEach(d=>{ const k=_bucketKey(d.fecha,chartAgg); if(k==null) return; const cur=m.get(k); if(!cur) m.set(k,{fecha:k,val:d.val}); else cur.val=isSum?Math.round(cur.val+d.val):Math.max(cur.val,d.val); }); return [...m.values()].sort((a,b)=>a.fecha<b.fecha?-1:1); })();
    const _cmp=(()=>{ if(chartRange==="all") return {mode:"none"}; const back=(d)=>{ if(chartRange==="30d")d.setDate(d.getDate()-30); else if(chartRange==="3m")d.setMonth(d.getMonth()-3); else if(chartRange==="1y")d.setFullYear(d.getFullYear()-1); }; const cutCur=new Date(); back(cutCur); cutCur.setHours(0,0,0,0); const cutPrev=new Date(cutCur); back(cutPrev); const win=(fecha,lo,hi)=>{ const t=new Date(fecha); if(isNaN(t.getTime())) return false; return t>=lo&&(hi==null||t<hi); }; const valOf=(ss)=>{ const vals=ss.map(h=>_sessionMetric(h,chartMetric)).filter(v=>v!=null); if(!vals.length) return null; return chartMetric==="vol"?Math.round(vals.reduce((a,b)=>a+b,0)):Math.max(...vals); }; const all=safeArr(cargas[histEj]); return {mode:"on",cur:valOf(all.filter(h=>win(h.fecha,cutCur,null))),prev:valOf(all.filter(h=>win(h.fecha,cutPrev,cutCur)))}; })();
    const _maOverlay=(chartMA&&(chartMetric==="peso"||chartMetric==="1rm")&&_chartSeries.length>1)?_chartSeries.map((d,i,arr)=>{ const w=arr.slice(Math.max(0,i-2),i+1); return Math.round((w.reduce((a,x)=>a+x.val,0)/w.length)*10)/10; }):null;
    return <div>
      {confirm&&<ConfirmDialog msg={confirm.msg} onConfirm={confirm.onConfirm} onCancel={()=>setConfirm(null)}/>}
      ...
      {_stats.has&&<Card style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:10,marginTop:4}}>
          {[["Peso máximo",`${_stats.heaviest}kg`],["1RM est. (Epley)",_stats.oneRM>0?`${_stats.oneRM}kg`:"–"],["Mejor volumen/set",_stats.bestSetVol>0?`${_stats.bestSetVol}kg`:"–"],["Mejor volumen/sesión",_stats.bestSessVol>0?`${_stats.bestSessVol}kg`:"–"],["Más reps en un set",_stats.mostReps>0?String(_stats.mostReps):"–"]].map(([k,v])=>(
        ...
        <p style={{color:C.textMuted,fontSize:10,margin:"8px 0 0",fontStyle:"italic"}}>Récords sobre series de trabajo (warm-ups excluidos). 1RM estimado con reps 1–12.</p>
      </Card>}
      {chartData.length>1&&<Card style={{marginBottom:12}}>
        <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10}}>
          {[["peso","Peso"],["1rm","1RM"],["vol","Volumen"],["reps","Reps"]].map(([k,lbl])=>(
            <button key={k} onClick={()=>setChartMetric(k)} style={{background:chartMetric===k?C.card:C.surface,border:`1px solid ${chartMetric===k?C.text:C.border}`,borderRadius:8,color:chartMetric===k?C.text:C.textSub,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:chartMetric===k?700:500,flex:"1 0 auto"}}>{lbl}</button>
          ))}
        </div>
        <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10}}>
        ...
        {(chartMetric==="peso"||chartMetric==="1rm")&&<div style={{display:"flex",justifyContent:"flex-end",marginBottom:6}}>
          <button onClick={()=>setChartMA(!chartMA)} title="Media móvil de 3 puntos" style={{background:chartMA?C.card:C.surface,border:`1px solid ${chartMA?C.text:C.border}`,borderRadius:8,color:chartMA?C.text:C.textSub,padding:"4px 10px",cursor:"pointer",fontSize:10,fontWeight:600}}>Media móvil (3) {chartMA?"ON":"OFF"}</button>
        </div>}
        <SectionLabel>Progresión · {_mMeta[chartMetric][0]}{chartAgg!=="sesion"?` · por ${chartAgg}`:""}</SectionLabel>
        <BarChart data={_chartSeries} color={plano.color} unit={_mMeta[chartMetric][1]} overlay={_maOverlay}/>
        <InsightBox data={_chartSeries} unit={_mMeta[chartMetric][1]}/>
        ...
        {_cmp.mode==="on"&&_cmp.cur!=null&&(_cmp.prev==null
          ...
          : (()=>{ const u=_mMeta[chartMetric][1]; const dlt=Math.round((_cmp.cur-_cmp.prev)*100)/100; const pct=_cmp.prev>0?Math.round((dlt/_cmp.prev)*100):null; const up=dlt>0,down=dlt<0; const col=up?C.green:down?C.orange:C.textMuted; const arrow=up?"↑":down?"↓":"·"; return (
        ...
        <div key={i} style={{padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
          {editEntry===i?(
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div>
                <p style={{color:C.textMuted,fontSize:11,margin:"0 0 2px"}}>{fmt(h.fecha)}</p>
                {normSets(h.series).length>0&&<p style={{color:C.textMuted,fontSize:11,margin:0}}>Series: {normSets(h.series).map(s=>{const mk=(s.type&&s.type!=="normal")?(({warmup:"W",drop:"D",failure:"F"})[s.type]+" "):"";return mk+(s.reps!=null?`${s.kg}×${s.reps}`:`${s.kg}kg`)+(s.rpe!=null?` @${s.rpe}`:"");}).join(" · ")}</p>}
                {(()=>{
    ...
    {safeArr(plano.ejercicios).map((ej,idx)=>{
      ...
      return <div key={ej.id} ref={el=>{ejRefs.current[ej.id]=el;}} style={{background:done?C.text+"0d":C.card,border:`1px solid ${done?C.text+"35":C.border}`,borderLeft:ej.groupId?`3px solid ${ssColor(ej.groupId)}`:`1px solid ${done?C.text+"35":C.border}`,borderRadius:12,padding:16,marginBottom:12,transition:"all 0.2s"}}>
        ...
        {!done&&!editMode&&<>
          ...
          {ej.bar&&<div style={{marginBottom:8}}>
