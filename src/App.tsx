...
const K = {
  ...
  nutProfile:"pg_nup", mealPlan:"pg_mp", shopping:"pg_sh", proteinLog:"pg_pl", recipes:"pg_recipes", weeklyReview:"pg_wrev", weeklyProposal:"pg_wprop", weeklyProposalBackup:"pg_wprop_bak", nutGoal:"pg_nutgoal", trainingTemplate:"pg_tpl", trainingTemplateBackup:"pg_tpl_bak", exlib:"pg_exlib", onboarded:"pg_onboarded",
};
...
function exLogType(ej){ if(ej&&ej.logType) return ej.logType; const t=ej&&ej.tipo; if(t==="Peso corporal") return "bodyweight_reps"; if(t==="Isométrico") return "duration"; return "weight_reps"; }
...
function fmtSet(s,logType){ const lt=logType||"weight_reps"; if(lt==="duration") return fmtSecs(s&&s.seconds); if(lt==="bodyweight_reps") return ((s&&s.reps!=null)?String(s.reps):"")+((s&&s.addedWeight>0)?` +${s.addedWeight}kg`:""); return (s&&s.reps!=null)?`${s.kg}×${s.reps}`:`${s&&s.kg}`; }
function entryHeadline(h,logType){ const lt=logType||"weight_reps"; const ns=normSets(h&&h.series,lt); if(lt==="duration"){ const m=Math.max(0,...ns.map(s=>s.seconds||0)); return m>0?fmtSecs(m):"—"; } if(lt==="bodyweight_reps"){ const m=Math.max(0,...ns.map(s=>s.reps||0)); return m>0?(m+" reps"):"—"; } return ((h&&h.kg)||0)+"kg"; }
...
function getLibrary(planos,exlib){
  ...
  Object.values(planos||{}).forEach(p=>safeArr(p&&p.ejercicios).forEach(e=>{
    ...
    lib[key]={key, nombre:((e&&e.nombre)||prev.nombre||""), grupo:((e&&e.grupo)||prev.grupo||""), tipo:((e&&e.tipo)||prev.tipo||""), equipo:((e&&e.equipo)||prev.equipo||""), logType:((e&&e.logType)||prev.logType||"")};
  }));
  ...
  Object.keys(cat).forEach(key=>{
    ...
    lib[key]={key, nombre:pick(m.nombre,base.nombre), grupo:pick(m.grupo,base.grupo), tipo:pick(m.tipo,base.tipo), equipo:pick(m.equipo,base.equipo), logType:pick(m.logType,base.logType)};
  });
...
function upsertExlib(key,meta){
  ...
  ["nombre","grupo","tipo","equipo","logType"].forEach(f=>{ if(m[f]!=null&&m[f]!=="") next[f]=m[f]; });
...
const EX_LOGTYPES=[["weight_reps","Peso + reps"],["bodyweight_reps","Peso corporal"],["duration","Tiempo"]];
...
const BACKUP_KEY_NAMES = ["tabs","cargas","medidas","gymPlanos","planWeek","objectives","rpe","snota","customFoods","supplements","nutProfile","mealPlan","shopping","proteinLog","recipes","nutGoal","trainingTemplate","exlib"];
...
function BarChart({data,color,unit="kg",overlay=null}){
...
function GymPanel({initPlanoKey=null,autoStart=false,onSessionStarted}){
  ...
  const [plateOpen,setPlateOpen]=useState({}); // panel plate calculator abierto por ejercicio
  ...
  const [creMeta,setCreMeta]=useState({grupo:"",tipo:"Fuerza",equipo:"",logType:"weight_reps"});
  const [confirm,setConfirm]=useState(null);
  const [editEntry,setEditEntry]=useState(null);
  ...
  function addEj(nombre,meta){
    ...
    const ex=getLibrary(planos,load(K.exlib)).find(x=>x.key===key)||{};
    ...
    const ej={id,nombre:nm,grupo,series:3,reps:"8-10",descanso:60,exerciseKey:key,tipo,equipo,logType};
    const p=planos[planoSel];
    savePlanos({...planos,[planoSel]:{...p,ejercicios:[...p.ejercicios,ej]}});
    upsertExlib(key,{nombre:nm,grupo,tipo,equipo,logType});
    setNewEjName(""); setCreOpen(false); setCreMeta({grupo:"",tipo:"Fuerza",equipo:"",logType:"weight_reps"});
  }
  ...
  function handleSaveEj(ejId,descanso,numSeries){
    ...
    if(lt==="bodyweight_reps"){
      sets=arr.map(o=>{ const r=parseInt(o&&o.reps); const aw=pkg(o&&o.addedWeight); const rp=o&&o.rpe; return {kg:0,reps:isNaN(r)?null:r,addedWeight:isNaN(aw)?0:(aw>0?aw:0),type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.reps!=null&&s.reps>0);
      if(sets.length<numSeries) return; vals=sets.map(s=>s.reps);
    } else if(lt==="duration"){
      sets=arr.map(o=>{ const sec=parseSecs(o&&o.secStr); const rp=o&&o.rpe; return {kg:0,seconds:sec,type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.seconds>0);
      if(sets.length<numSeries) return; vals=sets.map(s=>s.seconds);
    } else {
      sets=arr.map(o=>{ const kg=pkg(o&&o.kg); const r=parseInt(o&&o.reps); const rp=o&&o.rpe; return {kg:isNaN(kg)?0:kg,reps:isNaN(r)?null:r,type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.kg>0);
    ...
    const trend=(lt==="weight_reps"&&prev)?(maxKg>prev?" ↑":maxKg<prev?" ↓":""):"";
  ...
  function deleteCargas(ejId,idx){
    const ejName=Object.values(planos).flatMap(p=>safeArr(p.ejercicios)).find(e=>e.id===ejId)?.nombre||"Ejercicio";
  ...
  if(histEj){
    ...
    const chartData=_aggHist.map(h=>({val:h.kg,fecha:h.fecha}));
    ...
    const _statRows=_lt==="duration"
      ? [["Hold máximo",_stats.longest>0?fmtSecs(_stats.longest):"–"],["Mejor tiempo/sesión",_stats.bestSess>0?fmtSecs(_stats.bestSess):"–"]]
      : _lt==="bodyweight_reps"
        ? [["Más reps en un set",_stats.topReps>0?String(_stats.topReps):"–"],["Mejor total reps/sesión",_stats.bestTotal>0?String(_stats.bestTotal):"–"]].concat(_stats.maxLastre>0?[["Lastre máximo",`${_stats.maxLastre}kg`]]:[])
        : [["Peso máximo",`${_stats.heaviest}kg`],["1RM est. (Epley)",_stats.oneRM>0?`${_stats.oneRM}kg`:"–"],["Mejor volumen/set",_stats.bestSetVol>0?`${_stats.bestSetVol}kg`:"–"],["Mejor volumen/sesión",_stats.bestSessVol>0?`${_stats.bestSessVol}kg`:"–"],["Más reps en un set",_stats.mostReps>0?String(_stats.mostReps):"–"]];
    ...
    const _metricDefs=_lt==="duration"?{hold:["Hold máx","s"],ttime:["Tiempo total","s"]}:_lt==="bodyweight_reps"?Object.assign({reps:["Reps top","reps"],treps:["Reps totales","reps"]},_hasLastre?{lastre:["Lastre máx","kg"]}:{}):{peso:["Peso top","kg"],"1rm":["1RM estimado","kg"],vol:["Volumen","kg"],reps:["Reps top","reps"]};
    ...
    const _sumMetric=(m)=>m==="vol"||m==="treps"||m==="ttime";
    const _recordStr=_lt==="duration"?(_stats.has?fmtSecs(_stats.longest):"–"):_lt==="bodyweight_reps"?(_stats.has?(_stats.topReps+" reps"):"–"):(maxVal>0?maxVal+"kg":"–");
    ...
    return <div>
      ...
      <Card accent={plano.color} style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:16,marginTop:8}}>
          {[["Récord",_recordStr],["Sesiones",_aggHist.length]].map(([k,v])=>(
            ...
            <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:12}}>
              {editSets.map((es,si)=>{
                ...
                {editRpePick===si&&<div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                  {[6,7,8,9,10].map(v=>(<button key={v} onClick={()=>{upd({rpe:v});setEditRpePick(null);}} style={{background:es.rpe===v?C.card:C.surface,border:`1px solid ${es.rpe===v?C.text:C.border}`,borderRadius:8,color:es.rpe===v?C.text:C.textSub,padding:"6px 14px",cursor:"pointer",fontSize:12,fontWeight:es.rpe===v?700:500,flex:"1 0 auto"}}>{v}</button>))}
  ...
  return <div>
    ...
    {safeArr(plano.ejercicios).length>0&&<div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
      <button onClick={()=>setRpeMode(!rpeMode)} title="Activar RPE por serie" style={{background:rpeMode?C.card:C.surface,border:`1px solid ${rpeMode?C.text:C.border}`,borderRadius:8,color:rpeMode?C.text:C.textSub,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:600}}>RPE {rpeMode?"ON":"OFF"}</button>
    </div>}
    {safeArr(plano.ejercicios).map((ej,idx)=>{
      ...
      const _prVal=(s)=>{ if((s&&(s.type||"normal"))==="warmup") return 0; if(lt==="duration") return s.seconds||0; if(lt==="bodyweight_reps") return ((s.addedWeight||0)===0)?(s.reps||0):0; return s.kg||0; };
      ...
      const _prCur=done
        ...
        : Math.max(0, ...sArr.map(o=>{ if((o&&(o.type||"normal"))==="warmup") return 0; if(lt==="duration") return parseSecs(o&&o.secStr)||0; if(lt==="bodyweight_reps") return (pkg(o&&o.addedWeight)>0)?0:(parseInt(o&&o.reps)||0); return pkg(o&&o.kg)||0; }));
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}>
              ...
              {isPR?<Tag color={C.yellow}>{lt==="duration"?"PR hold":lt==="bodyweight_reps"?"PR reps":"PR"}</Tag>:isVolPR?<Tag color={C.yellow}>PR vol</Tag>:null}
        ...
        {!done&&!editMode&&<>
          ...
          {lt==="weight_reps"&&<div style={{marginBottom:8}}>
            {Array.from({length:ej.series},(_,si)=>{
              ...
              return <div key={si}><div style={{display:"grid",gridTemplateColumns:"70px 28px minmax(0,1fr) auto 50px auto auto",gap:5,alignItems:"stretch",minHeight:36,marginBottom:6}}>
                ...
                {rpeMode&&<button onClick={()=>setRpePick(rpePick===(ej.id+":"+si)?null:(ej.id+":"+si))} title="RPE de la serie" style={{background:cur.rpe?C.card:C.surface,border:`1px solid ${cur.rpe?C.text:C.border}`,borderRadius:8,color:cur.rpe?C.text:C.textMuted,alignSelf:"stretch",boxSizing:"border-box",display:"flex",alignItems:"center",justifyContent:"center",padding:"0 8px",cursor:"pointer",fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{cur.rpe?("@"+cur.rpe):"RPE"}</button>}
              </div>
              {rpeMode&&rpePick===(ej.id+":"+si)&&<div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                {[6,7,8,9,10].map(v=>(<button key={v} onClick={()=>{setCur({rpe:v});setRpePick(null);}} style={{background:cur.rpe===v?C.card:C.surface,border:`1px solid ${cur.rpe===v?C.text:C.border}`,borderRadius:8,color:cur.rpe===v?C.text:C.textSub,padding:"6px 14px",cursor:"pointer",fontSize:12,fontWeight:cur.rpe===v?700:500,flex:"1 0 auto"}}>{v}</button>))}
          ...
          {lt!=="weight_reps"&&<div style={{marginBottom:8}}>
            {Array.from({length:ej.series},(_,si)=>{
              ...
              const refN=normSets(last?.series,lt)[si];
              const ok=lt==="duration"?parseSecs(cur.secStr)>0:((parseInt(cur.reps)||0)>0);
              const iStyle=(active)=>({flex:1,background:active?C.green+"18":C.surface,border:`1px solid ${active?C.green+"55":C.border}`,borderRadius:8,color:C.text,padding:"0 8px",height:34,boxSizing:"border-box",outline:"none",fontFamily:"inherit",minWidth:0});
              return <div key={si}>
                <div style={{display:"flex",gap:6,alignItems:"center",minHeight:36,marginBottom:6}}>
                  <span style={{color:C.textMuted,fontSize:11,fontWeight:500,width:44,flexShrink:0}}>S{si+1}</span>
                  {lt==="duration"
                    ? <input type="text" inputMode="numeric" value={cur.secStr||""} onChange={e=>setCur({secStr:e.target.value.replace(/[^0-9:]/g,"")})} onKeyDown={e=>{if(e.key==="Enter"&&ok){e.preventDefault();confirmSet(ej,cur);}}} placeholder={refN?fmtSecs(refN.seconds):"mm:ss"} style={iStyle(ok)}/>
                  ...
                  <span style={{color:C.textMuted,fontSize:10,flexShrink:0}}>{lt==="duration"?"tiempo":"reps"}</span>
                  {lt==="bodyweight_reps"&&<input type="text" inputMode="decimal" value={cur.addedWeight||""} onChange={e=>setCur({addedWeight:e.target.value.replace(/[^0-9.,]/g,"")})} placeholder="lastre" style={{width:64,flexShrink:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,padding:"0 8px",height:34,boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/>}
                  {lt==="bodyweight_reps"&&<span style={{color:C.textMuted,fontSize:10,flexShrink:0}}>kg</span>}
                  <button onClick={()=>{if(ok)confirmSet(ej,cur);}} title="Confirmar serie" style={{background:ok?C.green+"22":C.surface,border:`1px solid ${ok?C.green:C.border}`,borderRadius:8,color:ok?C.green:C.textMuted,height:34,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 10px",cursor:"pointer",fontSize:13,fontWeight:700,opacity:ok?1:0.45}}>✓</button>
                  {rpeMode&&<button onClick={()=>setRpePick(rpePick===(ej.id+":"+si)?null:(ej.id+":"+si))} title="RPE" style={{background:cur.rpe?C.card:C.surface,border:`1px solid ${cur.rpe?C.text:C.border}`,borderRadius:8,color:cur.rpe?C.text:C.textMuted,height:34,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 8px",cursor:"pointer",fontSize:11,fontWeight:700}}>{cur.rpe?("@"+cur.rpe):"RPE"}</button>}
                </div>
                {rpeMode&&rpePick===(ej.id+":"+si)&&<div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                  {[6,7,8,9,10].map(v=>(<button key={v} onClick={()=>{setCur({rpe:v});setRpePick(null);}} style={{background:cur.rpe===v?C.card:C.surface,border:`1px solid ${cur.rpe===v?C.text:C.border}`,borderRadius:8,color:cur.rpe===v?C.text:C.textSub,padding:"6px 14px",cursor:"pointer",fontSize:12,fontWeight:cur.rpe===v?700:500,flex:"1 0 auto"}}>{v}</button>))}
                  <button onClick={()=>{setCur({rpe:null});setRpePick(null);}} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.textMuted,padding:"6px 14px",cursor:"pointer",fontSize:12}}>limpiar</button>
    ...
    {editMode&&<div style={{marginTop:8}}>
      ...
      {newEjName.trim()&&(()=>{
        ...
        const lib=getLibrary(planos,load(K.exlib));
        const sc=x=>{const n=normalizeName(x.nombre);return n===q?0:n.indexOf(q)===0?1:n.indexOf(q)>=0?2:9;};
        const matches=lib.filter(x=>normalizeName(x.nombre).indexOf(q)>=0).sort((a,b)=>sc(a)-sc(b)||String(a.nombre).localeCompare(String(b.nombre))).slice(0,6);
        const exact=lib.find(x=>x.key===makeExerciseKey(newEjName));
        ...
        return <div style={{marginTop:6,display:"flex",flexDirection:"column",gap:4}}>
          ...
          {(!exact&&creOpen)&&<div style={{display:"flex",flexDirection:"column",gap:6,padding:8,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,marginTop:2}}>
