                ...
                {isEmptyMeal&&mealSugs.length>0&&<div style={{marginBottom:5}}>
                  ...
                  {mealSugs.map(s=>(
                    <button key={s.name} onClick={()=>{const upd={...mealPlan};const ms=[...((upd[di]?.meals)||MEAL_TYPES.map(t=>({t,desc:"",prot:"",kcal:""})))];ms[mi]={...ms[mi],desc:s.name,prot:s.prot,kcal:s.kcal};upd[di]={...upd[di],meals:ms};saveMealPlan(upd);}}
                      style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:8,width:"100%",background:"none",border:"none",borderBottom:`1px solid ${C.border}`,padding:"6px 2px",cursor:"pointer",textAlign:"left"}}>
                      <span style={{color:C.text,fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</span>
