  ...
  return <div>
    ...
    <Card style={{marginBottom:16,background:color+"0e",border:`1px solid ${color}33`}}>
      ...
      {tab.id==="powerlifting"&&<div style={{background:C.orange+"18",border:`1px solid ${C.orange}44`,borderRadius:8,padding:"8px 10px",marginBottom:10,display:"flex",gap:8,alignItems:"center"}}>
        ...
        <p style={{color:C.orange,fontSize:11,margin:0}}>Calentamiento obligatorio antes de levantar — 5-10min movilidad + series progresivas al 40-60% del peso objetivo.</p>
      </div>}
...
function ScreenHoy({onNavigate}){
  ...
  return <div style={{paddingBottom:8}}>
    ...
    {(()=>{
      ...
      const sl=Math.round((Math.round(10*pw+6.25*np.altura-5*np.edad+5))*np.actividad)+sx;
      const pt=Math.round(pw*2);
      const ht=Math.round(pw*40/1000*10)/10;
      const pl=(load(K.proteinLog)||{})[todayDate]||[];
      const ph=pl.reduce((a,e)=>a+(e.prot||0),0);
      const hh=(load(K.proteinLog)||{})[todayDate+"_h"]||0;
      const fh=(load(K.proteinLog)||{})[todayDate+"_f"]||0;
      const sdone=Object.values((load("pg_sup_done")||{})[todayDate]||{}).filter(Boolean).length;
      const stot=(load(K.supplements)||[]).length;
      const slp=(load(K.proteinLog)||{})[todayDate+"_sleep"]||{};
      const nc="#f97316";
      return <Card style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <p style={{color:C.text,fontSize:13,fontWeight:700,margin:0}}>🥗 Nutrición hoy</p>
          <Tag color={nc}>{sl} kcal</Tag>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          {[[ph,pt,"g","💪",nc],[parseFloat(hh.toFixed(1)),ht,"L","💧",C.blue],[fh,32,"g","🌿",C.green]].map(([cur,tgt,unit,icon,col],ki)=>{
            const pp=Math.min(100,Math.round((cur/tgt)*100));
            return <div key={ki} style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                <p style={{color:C.textMuted,fontSize:10,margin:0}}>{icon}</p>
                <p style={{color:pp>=100?C.green:C.text,fontSize:10,fontWeight:700,margin:0}}>{cur}/{tgt}{unit}</p>
              </div>
              <ProgressBar pct={pp} color={pp>=100?C.green:col} height={4}/>
            </div>;
          })}
        </div>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          {stot>0&&<p style={{color:sdone===stot?C.green:C.textMuted,fontSize:11,margin:0}}>💊 {sdone}/{stot} suplementos</p>}
          {slp.rating>0&&<p style={{color:slp.rating<=2?C.red:slp.rating===3?C.yellow:C.green,fontSize:11,margin:0}}>{["😴","💤","😐","😊","🌟"][slp.rating-1]} Sueño{slp.hours?` ${slp.hours}h`:""}</p>}
        </div>
        {ph/pt<0.3&&<p style={{color:C.orange,fontSize:11,margin:"5px 0 0"}}>⚡ Proteína baja — prioridad del día</p>}
      </Card>;
    })()}
    ...
    {(stalledEjs.length>0||cfStalledEjs.length>0)&&<Card accent={C.red} style={{marginBottom:14}}>
      ...
      {stalledEjs.map(ej=><div key={ej.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
      ...
      {cfStalledEjs.map(ej=><div key={ej.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
    ...
    <Card style={{marginBottom:16}}>
      <button onClick={()=>setShowWeekPlan(s=>!s)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:0}}>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <p style={{color:C.text,fontSize:13,fontWeight:700,margin:0}}>📅 Plan semanal</p>
          {streak>1&&<Tag color={C.yellow}>🔥 {streak} sem</Tag>}
          <Tag color={C.green}>{daysThisWeek} días</Tag>
        </div>
        <span style={{color:C.textMuted,fontSize:14}}>{showWeekPlan?"▲":"▼"}</span>
      </button>
    ...
    <Card style={{marginBottom:16}}>
      <button onClick={()=>setShowObjetivos(s=>!s)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:0}}>
        <p style={{color:C.text,fontSize:13,fontWeight:700,margin:0}}>🎯 Objetivos</p>
        <span style={{color:C.textMuted,fontSize:14}}>{showObjetivos?"▲":"▼"}</span>
      </button>
...
function BackupPanel({onClose,setImportMsg}){
  const [mode,setMode]=useState("menu"); // menu | export | paste
  const [jsonText,setJsonText]=useState("");
  const [pasteText,setPasteText]=useState("");
  const [copied,setCopied]=useState(false);
  function handleExport(){setJsonText(exportData());setMode("export");}
  function handleCopy(){
  ...
  function handlePasteImport(){
    ...
    try{
      ...
      if(!p.data) throw new Error("Formato inválido");
      Object.entries(p.data).forEach(([k,v])=>save(k,v));
      setImportMsg("✓ Datos importados");
      onClose();
      setTimeout(()=>window.location.reload(),800);
    }catch(e){setImportMsg("✗ Error: "+e.message);setTimeout(()=>setImportMsg(null),3000);}
  }
  return <div style={{maxWidth:500,margin:"10px auto 0",padding:"14px",background:C.card,borderRadius:14,border:`1px solid ${C.border}`}}>
    ...
    {mode==="menu"&&<>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <button onClick={handleExport} style={{background:C.gymColor,color:"#000",border:"none",borderRadius:10,padding:"12px",fontSize:13,fontWeight:700,cursor:"pointer"}}>📋 Exportar — Ver JSON</button>
        <button onClick={()=>setMode("paste")} style={{background:C.blue+"22",color:C.blue,border:`1px solid ${C.blue}44`,borderRadius:10,padding:"12px",fontSize:13,fontWeight:700,cursor:"pointer"}}>📥 Importar — Pegar JSON</button>
        <label style={{background:C.surface,color:C.textSub,border:`1px solid ${C.borderLight}`,borderRadius:10,padding:"12px",fontSize:13,fontWeight:700,cursor:"pointer",textAlign:"center"}}>
          📂 Importar — Desde archivo<input type="file" accept=".json" style={{display:"none"}} onChange={e=>{if(!e.target.files[0])return;importData(e.target.files[0],ok=>{setImportMsg(ok?"✓ Datos importados":"✗ Error");onClose();setTimeout(()=>setImportMsg(null),3000);});}}/>
        </label>
      </div>
    </>}
    {mode==="export"&&<>
      ...
      <button onClick={()=>setMode("menu")} style={{background:"none",border:"none",color:C.textMuted,fontSize:12,cursor:"pointer",padding:"6px 0 0"}}>← Volver</button>
    </>}
    {mode==="paste"&&<>
      <p style={{color:C.textSub,fontSize:12,margin:"0 0 8px"}}>Pega el JSON de tu backup:</p>
      <textarea value={pasteText} onChange={e=>setPasteText(e.target.value)} placeholder='{"version":2,"data":{...}}' style={{width:"100%",height:160,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontSize:11,padding:10,boxSizing:"border-box",resize:"none",fontFamily:"monospace",marginBottom:8}}/>
      <div style={{display:"flex",gap:8}}>
        <Btn onClick={handlePasteImport} color={C.blue} style={{flex:1}}>Importar</Btn>
        <button onClick={()=>setMode("menu")} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:10,color:C.textMuted,padding:"10px 14px",cursor:"pointer",fontSize:12}}>Cancelar</button>
      </div>
    </>}
  </div>;
}
...
function ScreenNutricion(){
  ...
  const [nutTab,setNutTab]=useState("hoynut");
  ...
  function saveFiber(val){setFiberToday(val);const all=load(K.proteinLog)||{};all[today()+"_f"]=val;save(K.proteinLog,all);}
  const [customFoods,setCustomFoods]=useState(()=>load(K.customFoods)||[]);
  const [supplements,setSupplements]=useState(()=>load(K.supplements)||[
    {id:"sup1",name:"Creatina",dose:"5g"},
    {id:"sup2",name:"Batido de proteínas",dose:"30g"},
  ]);
  ...
  function saveProteinLog(upd){
    setProteinLog(upd);
    const all=load(K.proteinLog)||{};
    all[today()]=upd;save(K.proteinLog,all);
  }
  function saveHydration(val){
    setHydration(val);
    const all=load(K.proteinLog)||{};
    all[today()+"_h"]=val;save(K.proteinLog,all);
  }
  ...
  const fiberTarget=32; // g/day for active male
  const isTrainingDay=getTrainedSessions(today()).length>0||loadWeek()[(new Date().getDay()+6)%7]?.assignments?.length>0;
  const hydroTarget=Math.round(pesoActual*(isTrainingDay?42:35)/1000*10)/10; // 42ml/kg training days, 35ml/kg rest
  const proteinToday=proteinLog.reduce((a,e)=>a+e.prot,0);
  const proteinPct=Math.min(100,Math.round((proteinToday/proteinTarget)*100));
  const hydroPct=Math.min(100,Math.round((hydration/hydroTarget)*100));
  ...
  const gainFast=gainRate&&parseFloat(gainRate)>0.5;
  ...
  return <div>
    {/* Tab bar */}
    <div style={{display:"flex",gap:6,marginBottom:18,overflowX:"auto"}}>
      {[["shop","🛒 Compra"],["hoynut","📅 Hoy"],["meals","🍽 Comidas"],["datos","📊 Datos"]].map(([id,label])=>(
        <button key={id} onClick={()=>setNutTab(id)} style={{flexShrink:0,background:nutTab===id?nutColor:C.card,color:nutTab===id?"#000":C.textSub,border:`1.5px solid ${nutTab===id?nutColor:C.border}`,borderRadius:10,padding:"9px 14px",fontSize:12,fontWeight:nutTab===id?700:400,cursor:"pointer",transition:"all 0.15s"}}>
          {label}
        </button>
      ))}
    </div>
    ...
    {nutTab==="meals"&&<div>
      {!lastTrainedToday&&loadWeek()[(new Date().getDay()+6)%7]?.assignments?.length>0&&(
        <div style={{background:C.blue+"15",border:`1px solid ${C.blue}44`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",gap:10,alignItems:"flex-start"}}>
          <span style={{fontSize:18}}>⚡</span>
          <div>
            <p style={{color:C.blue,fontSize:13,fontWeight:700,margin:"0 0 2px"}}>Hoy toca entrenar — recuerda la comida pre-entreno</p>
            <p style={{color:C.textSub,fontSize:12,margin:0}}>1-2h antes: <strong>40-60g carbos + 20g proteína</strong> (avena+leche, arroz+pollo). Mejora rendimiento y reduce catabolismo.</p>
          </div>
        </div>
      )}
      {lastTrainedToday&&<div style={{background:C.green+"15",border:`1px solid ${C.green}44`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",gap:10,alignItems:"flex-start"}}>
        <span style={{fontSize:18}}>💪</span>
        <div>
          <p style={{color:C.green,fontSize:13,fontWeight:700,margin:"0 0 2px"}}>Entrenaste hoy — recuerda la comida post-entreno</p>
          <p style={{color:C.textSub,fontSize:12,margin:0}}>En los próximos 60 min: <strong>30-40g proteína + 60-80g carbos</strong> (arroz, avena, fruta) para maximizar la recuperación.</p>
        </div>
      </div>}
      ...
      {dayNames.map((dayName,di)=>{
        ...
        return <Card key={di} style={{marginBottom:10,border:isToday?`1.5px solid ${nutColor}`:undefined}}>
            ...
            return <div key={mi} style={{marginBottom:7}}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <Input type="text" value={val} onChange={e=>{
                  ...
                  upd[di]={...upd[di],meals:meals.map((m,idx2)=>idx2===mi?{...m,desc:e.target.value}:m)};
                  saveMealPlan(upd);
                }} placeholder="Ej: Avena con fruta y huevos" style={{flex:1,fontSize:12,padding:"7px 12px"}}/>
                <div style={{display:"flex",gap:3}}>
                  {[["🥩","P"],["🌾","C"],["⚖️","M"]].map(([icon,code])=>{
                    const cur=(mealPlan[di]?.meals||[])[mi]?.protTag||"";
                    return <button key={code} title={code==="P"?"Alta proteína":code==="C"?"Alta carbos":"Mixta"} onClick={()=>{
                      const upd={...mealPlan};
                      const meals=upd[di]?.meals||MEAL_TYPES.map(t=>({t,desc:"",protTag:""}));
                      upd[di]={...upd[di],meals:meals.map((m,idx2)=>idx2===mi?{...m,protTag:cur===code?"":code}:m)};
                      saveMealPlan(upd);
                    }} style={{background:cur===code?code==="P"?C.red+"33":code==="C"?C.yellow+"33":C.blue+"22":C.surface,border:`1px solid ${cur===code?code==="P"?C.red:code==="C"?C.yellow:C.blue:C.border}`,borderRadius:6,padding:"4px 6px",cursor:"pointer",fontSize:12}}>{icon}</button>;
                  })}
                </div>
              </div>
            </div>;
          ...
          {di===todayDow&&(()=>{
            ...
            const protMeals=meals.filter(m=>m.protTag==="P"||m.protTag==="M").length;
            const pActual3=load(K.medidas)?.length?parseFloat(load(K.medidas)[0].peso)||65.7:65.7;
            const protTarget3=Math.round(pActual3*2);
            if(protMeals>=4) return <p style={{color:C.green,fontSize:11,margin:"6px 0 0"}}>✅ Buena distribución ({protMeals} comidas con proteína)</p>;
            if(protMeals>=2) return <p style={{color:C.orange,fontSize:11,margin:"6px 0 0"}}>⚡ Objetivo: 4-5 comidas con ~{Math.round(protTarget3/5)}g proteína cada una</p>;
            return <p style={{color:C.textMuted,fontSize:11,margin:"6px 0 0",fontStyle:"italic"}}>Marca 🥩 en cada comida con proteína para ver distribución</p>;
          })()}
        </Card>;
      })}
    </div>}

    {/* ── HOY — SEGUIMIENTO DIARIO ── */}
    {nutTab==="hoynut"&&<div>
      {/* Protein tracker */}
      <Card style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
          <p style={{color:nutColor,fontSize:14,fontWeight:700,margin:0}}>💪 Proteína</p>
          <p style={{color:C.text,fontSize:22,fontWeight:800,margin:0}}>{proteinToday}<span style={{fontSize:12,color:C.textMuted}}>/{proteinTarget}g</span></p>
        </div>
        <ProgressBar pct={proteinPct} color={proteinPct>=100?C.green:nutColor} height={8}/>
        <p style={{color:C.textMuted,fontSize:11,margin:"5px 0 12px"}}>
          {proteinPct>=100?"✅ Objetivo alcanzado":proteinPct>=70?"Casi — faltan "+(proteinTarget-proteinToday)+"g":"Faltan "+(proteinTarget-proteinToday)+"g · prioridad del día"}
        </p>
        <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
          {PROTEIN_FOODS.map(f=>(
            <button key={f.name} onClick={()=>saveProteinLog([...proteinLog,{name:f.name,prot:f.prot,time:new Date().toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"})}])} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"5px 10px",cursor:"pointer",fontSize:10,color:C.textSub}}>
              +{f.prot}g {f.name}
            </button>
          ))}
        </div>
        {customFoods.length>0&&<div style={{marginBottom:8}}>
          <p style={{color:C.textMuted,fontSize:9,letterSpacing:2,textTransform:"uppercase",margin:"0 0 5px"}}>Mis favoritos</p>
          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
            {customFoods.map(f=>(
              <button key={f.id} onClick={()=>saveProteinLog([...proteinLog,{name:f.name,prot:f.prot,time:new Date().toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"})}])} style={{background:nutColor+"18",border:`1px solid ${nutColor}44`,borderRadius:8,padding:"5px 10px",cursor:"pointer",fontSize:10,color:nutColor,fontWeight:600}}>
                ⭐ +{f.prot}g {f.name}
              </button>
            ))}
          </div>
        </div>}
        <CustomProteinAdd onAdd={entry=>saveProteinLog([...proteinLog,entry])} color={nutColor} onSaveFood={food=>saveCustomFoods([...customFoods,food])}/>
        {proteinLog.length>0&&<div style={{borderTop:`1px solid ${C.border}`,paddingTop:8,marginTop:4}}>
          {proteinLog.slice().reverse().map((e,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
              <div><p style={{color:C.text,fontSize:12,margin:0}}>{e.name}</p><p style={{color:C.textMuted,fontSize:9,margin:0}}>{e.time}</p></div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <p style={{color:nutColor,fontSize:13,fontWeight:700,margin:0}}>+{e.prot}g</p>
                <button onClick={()=>{const upd=[...proteinLog];upd.splice(proteinLog.length-1-i,1);saveProteinLog(upd);}} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:12}}>🗑</button>
              </div>
            </div>
          ))}
        </div>}
      ...
      <Card style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
          <p style={{color:C.blue,fontSize:14,fontWeight:700,margin:0}}>💧 Hidratación</p>
          <p style={{color:C.text,fontSize:22,fontWeight:800,margin:0}}>{hydration.toFixed(1)}<span style={{fontSize:12,color:C.textMuted}}>/{hydroTarget}L</span></p>
        </div>
        <ProgressBar pct={hydroPct} color={hydroPct>=100?C.green:C.blue} height={8}/>
        <div style={{display:"flex",gap:6,marginTop:10}}>
          {[0.25,0.33,0.5,1].map(vol=>(
            <button key={vol} onClick={()=>saveHydration(Math.round((hydration+vol)*10)/10)} style={{flex:1,background:C.surface,border:`1px solid ${C.borderLight}`,borderRadius:8,padding:"9px 4px",cursor:"pointer",fontSize:11,color:C.textSub,fontWeight:600}}>+{vol}L</button>
          ))}
          {hydration>0&&<button onClick={()=>saveHydration(0)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 8px",cursor:"pointer",fontSize:11,color:C.red}}>✕</button>}
        </div>
      </Card>
      ...
      <Card style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <p style={{color:C.text,fontSize:14,fontWeight:700,margin:0}}>💊 Suplementos</p>
          <button onClick={()=>{const id="sup_"+Date.now();saveSupplements([...supplements,{id,name:"Nuevo",dose:""}]);}} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.textSub,padding:"4px 10px",cursor:"pointer",fontSize:11}}>+ Añadir</button>
        </div>
        {supplements.map((sup,i)=>(
          <div key={sup.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
            <button onClick={()=>{
            ...
            }} style={{width:24,height:24,borderRadius:6,flexShrink:0,background:supDone[sup.id]?C.green:C.surface,border:`2px solid ${supDone[sup.id]?C.green:C.borderLight}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#000",fontSize:14,fontWeight:700}}>
              {supDone[sup.id]?"✓":""}
            </button>
            <div style={{flex:1}}>
              <Input type="text" value={sup.name} onChange={e=>saveSupplements(supplements.map((s,si)=>si===i?{...s,name:e.target.value}:s))} style={{fontSize:13,padding:"5px 8px",marginBottom:3}}/>
              <Input type="text" value={sup.dose||""} onChange={e=>saveSupplements(supplements.map((s,si)=>si===i?{...s,dose:e.target.value}:s))} placeholder="Dosis (ej: 5g, 30g proteína)" style={{fontSize:11,padding:"4px 8px"}}/>
            </div>
            <button onClick={()=>saveSupplements(supplements.filter((_,si)=>si!==i))} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:13}}>🗑</button>
          </div>
        ))}
        <p style={{color:C.textMuted,fontSize:10,margin:"8px 0 0",fontStyle:"italic"}}>
          Los suplementos con gramos de proteína (ej: "30g") suman al tracker automáticamente.
          <br/>💡 Creatina: tómala post-entreno con carbos para mejorar la absorción ~60%.
        </p>
      </Card>
      ...
      <Card style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          ...
          {sleepData.rating>0&&<Tag color={sleepData.rating>=4?C.green:sleepData.rating>=3?C.yellow:C.red}>{["😴","💤","😐","😊","🌟"][sleepData.rating-1]} {sleepData.hours?sleepData.hours+"h":""}</Tag>}
        </div>
        <div style={{display:"flex",gap:5,marginBottom:8}}>
          {[1,2,3,4,5].map(n=>{
            const sc=n<=2?C.red:n===3?C.yellow:C.green;
            const labels=["Muy mal","Mal","Regular","Bien","Muy bien"];
            return <button key={n} onClick={()=>saveSleep({...sleepData,rating:sleepData.rating===n?0:n})} style={{flex:1,background:sleepData.rating===n?sc+"22":C.surface,border:`1.5px solid ${sleepData.rating===n?sc:C.border}`,borderRadius:8,padding:"8px 3px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
              <span style={{fontSize:16}}>{["😴","💤","😐","😊","🌟"][n-1]}</span>
              <p style={{color:sleepData.rating===n?sc:C.textMuted,fontSize:8,margin:0}}>{labels[n-1]}</p>
            </button>;
          })}
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <p style={{color:C.textMuted,fontSize:12,margin:0,flexShrink:0}}>Horas:</p>
          <Input value={sleepData.hours} onChange={e=>saveSleep({...sleepData,hours:e.target.value})} placeholder="Ej: 7.5" style={{flex:1,fontSize:12,padding:"7px 12px"}}/>
        </div>
        {sleepData.rating>0&&sleepData.rating<=2&&<p style={{color:C.red,fontSize:11,margin:"8px 0 0"}}>⚠️ Sueño deficiente — síntesis proteica reducida hasta 60%. Considera bajar la intensidad hoy.</p>}
        {sleepData.rating>=4&&<p style={{color:C.green,fontSize:11,margin:"8px 0 0"}}>✅ Buen descanso — condiciones óptimas para crecer.</p>}
      </Card>
    </div>}

    {/* ── DATOS — CALCULADORA Y PERFIL ── */}
    {nutTab==="datos"&&<div>
      {/* Calorie hero */}
      <div style={{background:`linear-gradient(135deg,${nutColor}18,${C.surface})`,border:`1px solid ${nutColor}44`,borderRadius:20,padding:18,marginBottom:14}}>
        <Tag color={nutColor}>Objetivo: ganancia de masa limpia</Tag>
        <p style={{color:C.text,fontSize:24,fontWeight:800,margin:"8px 0 2px"}}>{surplus} kcal/día</p>
        <p style={{color:C.textMuted,fontSize:11,margin:"0 0 14px"}}>TDEE {TDEE} kcal + {surplusExtra} superávit ({grasaActual}% grasa) · {pesoActual}kg</p>
        ...
        <div style={{display:"flex",gap:6,marginTop:6}}>
          <div style={{flex:1,background:C.card,borderRadius:10,padding:"8px 6px",textAlign:"center"}}>
            <p style={{color:C.textMuted,fontSize:9,margin:"0 0 2px"}}>💧 Agua objetivo</p>
            <p style={{color:C.blue,fontSize:14,fontWeight:700,margin:0}}>{hydroTarget}L</p>
          </div>
          <div style={{flex:1,background:C.card,borderRadius:10,padding:"8px 6px",textAlign:"center"}}>
            <p style={{color:C.textMuted,fontSize:9,margin:"0 0 2px"}}>🌿 Fibra objetivo</p>
            <p style={{color:C.green,fontSize:14,fontWeight:700,margin:0}}>{fiberTarget}g</p>
          </div>
        </div>
      </div>
      ...
      {gainRate!==null&&<Card style={{marginBottom:14}}>
        ...
        <p style={{color:gainOk?C.green:gainSlow?C.orange:C.red,fontSize:12,margin:"0 0 4px",fontWeight:700}}>
          {gainOk?"✅ Ritmo ideal — ganando masa limpia":gainSlow?"⚠️ Muy lento — sube 100-200 kcal/día":"⚠️ Demasiado rápido — reduce 100 kcal para menos grasa"}
        </p>
        <p style={{color:C.textMuted,fontSize:11,margin:0}}>Rango óptimo: 0.15–0.5 kg/semana. {pesoInicial}kg → {pesoActual}kg</p>
      </Card>}
      {gainRate===null&&<Card style={{marginBottom:14}}>
        <p style={{color:C.textSub,fontSize:13,fontWeight:600,margin:"0 0 4px"}}>📊 Velocidad de ganancia</p>
        <p style={{color:C.textMuted,fontSize:12,margin:0}}>Necesitas al menos 2 evaluaciones físicas para calcular tu ritmo de ganancia. Añádelas en Plan → Medidas.</p>
      </Card>}
      ...
      <Card style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          <div style={{flex:1}}><p style={{color:C.textMuted,fontSize:10,margin:"0 0 4px"}}>Altura (cm)</p><Input value={profile.altura} onChange={e=>saveProfile({...profile,altura:parseInt(e.target.value)||175})} placeholder="175"/></div>
          <div style={{flex:1}}><p style={{color:C.textMuted,fontSize:10,margin:"0 0 4px"}}>Edad</p><Input value={profile.edad} onChange={e=>saveProfile({...profile,edad:parseInt(e.target.value)||31})} placeholder="31"/></div>
        </div>
        ...
        {[[1.2,"Sedentario"],[1.375,"Ligero — 1-3 días/sem"],[1.55,"Moderado — 3-5 días/sem"],[1.725,"Activo — 6-7 días/sem"],[1.9,"Muy activo — 2x/día"]].map(([val,label])=>(
          <button key={val} onClick={()=>saveProfile({...profile,actividad:val})} style={{display:"block",width:"100%",background:profile.actividad===val?nutColor+"22":C.surface,color:profile.actividad===val?nutColor:C.textSub,border:`1px solid ${profile.actividad===val?nutColor+"66":C.border}`,borderRadius:8,padding:"7px 12px",cursor:"pointer",fontSize:11,textAlign:"left",fontWeight:profile.actividad===val?700:400,marginBottom:4}}>
            {profile.actividad===val?"✓ ":""}{label}
          </button>
        ))}
        <p style={{color:C.textMuted,fontSize:10,margin:"8px 0 0",fontStyle:"italic"}}>Peso y grasa de última evaluación: {pesoActual}kg / {grasaActual}% grasa</p>
      </Card>
    </div>}
  </div>;
}
...
function AppInner(){
  ...
  const titles={hoy:`${greeting}, Alvaro`,entreno:"Entrenamiento",plan:"Mi Plan",nutricion:"Nutrición",calendario:"Calendario"};
  ...
  return <div style={{fontFamily:"-apple-system,'SF Pro Display',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,paddingBottom:75,transition:"background 0.3s,color 0.3s"}}>
    ...
    <div style={{padding:"16px 20px 0",maxWidth:500,margin:"0 auto"}}>
      {screen==="hoy"&&<ScreenHoy onNavigate={(tab,planoKey)=>{setEntrenTab(tab);setEntrenPlano(planoKey||null);setScreen("entreno");}}/>}
