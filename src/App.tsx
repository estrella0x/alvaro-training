    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <div style={{display:"flex",gap:8}}>
        {Object.entries(planos).map(([k,p])=>(
          <button key={k} onClick={()=>{setPlanoSel(k);setSessionSaved({});setSeriesInputs({});setSessionRPE(null);setSessionNota("");}} style={{background:planoSel===k?C.accent:C.surface,color:planoSel===k?"#fff":C.textSub,border:`1px solid ${planoSel===k?C.accent:C.border}`,borderRadius:8,padding:"8px 16px",cursor:"pointer",fontSize:13,fontWeight:planoSel===k?600:400,transition:"all 0.15s"}}>
            {k}
          </button>
        ))}
      </div>
...
function SuenoHoy(){
  ...
  const [expanded,setExpanded]=useState(()=>{ const s=(load(K.proteinLog)||{})[today()+"_sleep"]||{}; return !s.rating; });
  function saveSleep(upd){setSleepData(upd);const all=load(K.proteinLog)||{};all[today()+"_sleep"]=upd;save(K.proteinLog,all);}
  const sc=sleepData.rating<=2?C.red:sleepData.rating===3?C.orange:C.green; // status color for collapsed summary
  ...
  return <Card style={{marginBottom:12}}>
    <button onClick={()=>setExpanded(e=>!e)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:0}}>
      <div style={{textAlign:"left"}}>
        <p style={{color:C.text,fontSize:13,fontWeight:600,margin:0,letterSpacing:-0.2}}>Sueño</p>
        {sleepData.rating>0
          ?<p style={{color:sc,fontSize:11,margin:"2px 0 0",fontWeight:400}}>{labels[sleepData.rating-1]}{sleepData.hours?` · ${sleepData.hours}h`:""}</p>
          :<p style={{color:C.textMuted,fontSize:11,margin:"2px 0 0"}}>Sin registrar</p>}
      </div>
      <span style={{color:C.textMuted,fontSize:12}}>{expanded?"▲":"▼"}</span>
    </button>
    {expanded&&<div style={{marginTop:14,borderTop:`1px solid ${C.border}`,paddingTop:14}}>
      <div style={{display:"flex",gap:5,marginBottom:12}}>
        {labels.map((label,i)=>{
          const n=i+1;const active=sleepData.rating===n;
          const c=n<=2?C.red:n===3?C.orange:C.green;
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        <Input value={sleepData.hours} onChange={e=>saveSleep({...sleepData,hours:e.target.value})} placeholder="7.5" style={{flex:1,fontSize:12,padding:"7px 10px"}}/>
      ...
      {sleepData.rating>0&&<button onClick={()=>setExpanded(false)} style={{marginTop:12,width:"100%",background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:11,padding:"4px 0",textAlign:"center"}}>Cerrar</button>}
    </div>}
  </Card>;
}
  ...
  return <div>
    ...
    {todayAssignments.length===0?(
      <Card style={{marginBottom:12,background:C.card}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>onEmpezar("skills",null)} style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.textSub,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:500}}> Skills ligero</button>
          <button onClick={onGoToNutricion} style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.textSub,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:500}}>Nutrición</button>
        </div>
      </Card>
    ):(
      <div style={{marginBottom:14}}>
        {todayAssignments.map((a,i)=>{
          ...
          return <div key={i} style={{background:C.card,border:`1.5px solid ${done?color+"66":color+"33"}`,borderRadius:14,padding:18,marginBottom:i<todayAssignments.length-1?10:0}}>
...
function ScreenCalendario(){
  ...
  return <div>
        ...
        {Array.from({length:diasEnMes},(_,i)=>i+1).map(day=>{
          ...
          return <button
            ...
            style={{
              aspectRatio:"1",
              borderRadius:8,
              border:`1.5px solid ${isSelected?C.accent:isToday?"#ffffff44":"transparent"}`,
    ...
    {/* Day detail panel */}
    {selectedDay&&(
      <Card style={{border:`1.5px solid ${C.accent}33`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
...
function ProgresoCuerpo(){
  ...
  return <div>
    ...
    {gainRate!==null?<Card style={{marginBottom:14}}>
      ...
      <p style={{color:gainOk?C.green:gainSlow?C.orange:C.red,fontSize:13,fontWeight:600,margin:"0 0 4px"}}>
