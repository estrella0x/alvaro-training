...
const BACKUP_KEY_NAMES = ["tabs","cargas","medidas","gymPlanos","planWeek","objectives","rpe","snota","customFoods","supplements","nutProfile","mealPlan","shopping","proteinLog","recipes","nutGoal","trainingTemplate","exlib","gymTpl","prog"];
...
function CFPanel({tab,onDeleteTab,autoStart=false,onSessionStarted}){
  ...
  function bsNewBlock(tipo){ return {id:Date.now()+Math.floor(Math.random()*1000),tipo:tipo||"warmup",titulo:"",contenido:"",resultado:"",rpe:"",kg:"",rounds:"",min:"",intervalo:"",rx:"",scoreType:"",fM:"",fS:"",wRounds:"",wReps:""}; }
  function loadBSessions(){ return Object.entries(data).filter(([k])=>k.startsWith("bsess_")).map(([,v])=>v).filter(Boolean).sort((a,b)=>(b.id||0)-(a.id||0)); }
  ...
  function bsLastWod(titulo,beforeId){ const t=normalizeName(titulo||""); if(!t) return null;
    for(const s of loadBSessions()){ if(beforeId&&s.id>=beforeId) continue;
      for(const b of (s.bloques||[])){ if(b&&b.tipo==="wod"&&normalizeName(b.titulo||"")===t&&(b.resultado||"").trim()) return {resultado:b.resultado,fecha:s.fecha,scoreType:b.scoreType||"",secs:b.secs,enc:(b.scoreType==="amrap")?((b.wRounds||0)*1000+(b.wReps||0)):null}; } }
    return null; }
  function bsWodCmp(curType,curVal,pv){ // 1=mejor, -1=peor, 0=no comparable/igual; solo mismo scoreType
...
function bsIndex(tipo){ const map={};
    loadBSessions().forEach(s=>(s.bloques||[]).forEach(b=>{ if(!b||b.tipo!==tipo) return; const k=normalizeName(b.titulo||""); if(!k) return;
      let e=map[k]; if(!e){ e=map[k]={key:k,display:b.titulo,count:0,lastFecha:s.fecha,lastSessId:s.id,entries:[]}; }
      e.count++; e.entries.push({fecha:s.fecha,sessId:s.id,contenido:b.contenido||"",kg:tipo==="fuerza"?safeNum(b.kg,0):0,scoreType:b.scoreType||"",secs:b.secs,enc:b.scoreType==="amrap"?((b.wRounds||0)*1000+(b.wReps||0)):null,resultado:b.resultado||""});
    }));
    return Object.values(map).sort((a,b)=>(b.lastSessId||0)-(a.lastSessId||0)); }
  ...
  function bsBest(item,tipo){ // criterio: fuerza max kg>0; wod dentro del scoreType del registro tipado mas reciente; sin comparables -> null
    ...
    const cmpE=item.entries.filter(e=>e.scoreType===ref.scoreType);
  ...
  function bsTrend(item,tipo){ // ≥2 puntos comparables o nada
    if(!item) return null;
    if(tipo==="fuerza"){ const pts=item.entries.filter(e=>e.kg>0).slice(0,8).reverse().map(e=>({fecha:e.fecha.slice(5),val:e.kg})); return pts.length>=2?{data:pts,unit:"kg",nota:null}:null; }
    const ref=item.entries.find(e=>e.scoreType); if(!ref) return null;
    if(ref.scoreType==="fortime"){ const pts=item.entries.filter(e=>e.scoreType==="fortime"&&e.secs>0).slice(0,8).reverse().map(e=>({fecha:e.fecha.slice(5),val:e.secs})); return pts.length>=2?{data:pts,unit:"s",nota:"menos es mejor"}:null; }
    const pts=item.entries.filter(e=>e.scoreType==="amrap"&&e.enc>0).slice(0,8).reverse().map(e=>({fecha:e.fecha.slice(5),val:Math.floor(e.enc/1000)}));
  ...
  function bsTitleSuggestions(tipo,q){ const qq=normalizeName(q||""); if(!qq) return [];
    ...
    loadBSessions().forEach(s=>(s.bloques||[]).forEach(b=>{ if(b&&b.tipo===tipo&&b.titulo) push(b.titulo); }));
  ...
  function editBSession(s){ setBsEditId(s.id); setBsFecha(s.fecha||today()); setBsNota(s.nota||""); setBsRPE(s.rpe?Number(s.rpe):null); setBsResOpen({}); setBsOptOpen({});
    setBsBlocks((s.bloques||[]).map((b,i)=>({id:Date.now()+i,tipo:b.tipo||"warmup",titulo:b.titulo||"",contenido:b.contenido||"",resultado:b.resultado||"",rpe:b.rpe||"",kg:b.kg||"",rounds:b.rounds||"",min:b.min||"",intervalo:b.intervalo||"",rx:b.rx||"",scoreType:b.scoreType||"",fM:(b.scoreType==="fortime"&&b.secs)?String(Math.floor(b.secs/60)):"",fS:(b.scoreType==="fortime"&&b.secs)?String(b.secs%60):"",wRounds:(b.scoreType==="amrap"&&b.wRounds!=null)?String(b.wRounds):"",wReps:(b.scoreType==="amrap"&&b.wReps!=null)?String(b.wReps):""})));
    setMode("blocks"); }
  function bsLastFuerza(titulo,beforeId){ const t=normalizeName(titulo||""); if(!t) return null;
    for(const s of loadBSessions()){ if(beforeId&&s.id>=beforeId) continue;
      for(const b of (s.bloques||[])){ if(b&&b.tipo==="fuerza"&&normalizeName(b.titulo||"")===t&&safeNum(b.kg,0)>0) return {kg:safeNum(b.kg,0),esquema:(b.contenido||"").split(String.fromCharCode(10))[0],fecha:s.fecha}; } }
    return null; }
  ...
  function saveBSSession(){ const bloques=bsBlocks.filter(b=>(""+(b.titulo||"")+(b.contenido||"")+(b.resultado||"")+(b.kg||"")+(b.fM||"")+(b.fS||"")+(b.wRounds||"")+(b.wReps||"")).trim()).map(b=>{ const o={tipo:b.tipo,titulo:(b.titulo||"").trim(),contenido:(b.contenido||"").trim(),resultado:(b.tipo==="warmup"||b.tipo==="fuerza")?"":(b.resultado||"").trim(),rpe:b.tipo==="wod"?(""+(b.rpe||"")).trim():"",kg:b.tipo==="fuerza"?(""+(b.kg||"")).trim():"",rounds:(b.tipo==="warmup"||b.tipo==="skill"||b.tipo==="fuerza")?(""+(b.rounds||"")).trim():"",min:(b.tipo==="warmup"||b.tipo==="skill")?(""+(b.min||"")).trim():"",intervalo:b.tipo==="fuerza"?(""+(b.intervalo||"")).trim():""};
      if(b.tipo==="wod"){ if(b.rx==="rx"||b.rx==="scaled") o.rx=b.rx; const st=b.scoreType||"";
        ...
        else if(st==="amrap"){ const r=parseInt(b.wRounds,10)||0, rp=parseInt(b.wReps,10)||0; if(r>0||rp>0){ o.scoreType="amrap"; o.wRounds=r; o.wReps=rp; o.resultado=r+" rds"+(rp>0?" + "+rp:""); } }
      }
  ...
  if(mode==="blocks"){
    ...
    return <div>
      ...
      {bsBlocks.map(b=>{ const isWod=b.tipo==="wod",isWarm=b.tipo==="warmup",isMid=b.tipo==="skill"||b.tipo==="fuerza"; const accent=isWod?color:isMid?C.textSub:C.border; const resOpen=!!bsResOpen[b.id]||!!(b.resultado&&b.resultado.length); return (<div key={b.id} style={{marginBottom:10,background:isWarm?C.surface:C.card,border:`1px solid ${C.border}`,borderLeft:`3px solid ${accent}`,borderRadius:10,padding:isWarm?"8px 10px":"11px 12px"}}>
        ...
        {b.tipo==="fuerza"&&(()=>{ const prev=bsLastFuerza(b.titulo,bsEditId); return <div style={{marginTop:6}}>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
        ...
        {isWod&&(()=>{ const pv=bsLastWod(b.titulo,bsEditId); if(!pv) return null;
          ...
          const cmp=bsWodCmp(st,cv,pv);
          const diff=(cmp!==0&&st==="fortime")?Math.abs(cv-pv.secs):null;
          return <p style={{color:C.textMuted,fontSize:10,margin:"4px 0 0"}}>{"Última vez: "}{pv.resultado} · {daysAgo(pv.fecha)}{cmp===1&&<span style={{color:C.green,fontWeight:700}}>{" ▲"}{diff!=null?` −${diff}s`:""}</span>}{cmp===-1&&<span style={{color:C.orange,fontWeight:700}}>{" ▼"}{diff!=null?` +${diff}s`:""}</span>}</p>; })()}
      </div>); })}
  ...
  if(mode==="bsindex"){
    ...
    idx.sort((a,b)=>((bench.has(b.key)?1:0)-(bench.has(a.key)?1:0))||((b.lastSessId||0)-(a.lastSessId||0)));
    return <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <BackBtn onClick={()=>setMode("list")}/>
  ...
  if(mode==="bsficha"&&bsFichaKey){
    ...
    return <div>
      ...
      {item.entries.map((e,i)=>{ const prev=item.entries[i+1]||null;
        ...
        else if(prev&&e.scoreType&&prev.scoreType===e.scoreType){ arrow=bsWodCmp(e.scoreType,e.scoreType==="fortime"?e.secs:e.enc,{scoreType:prev.scoreType,secs:prev.secs,enc:prev.enc}); }
  ...
  return <div>
    ...
    <Card style={{marginBottom:12,background:C.surface,border:`1px solid ${C.border}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
        ...
        {tab.id!=="wod"&&<SessionTimer timerId={tab.id} color={color} autoStart={autoStart} onStarted={onSessionStarted}/>}
      </div>
    ...
    {tab.id==="wod"&&(()=>{ const sess=loadBSessions(); if(!sess.length) return <p style={{color:C.textMuted,fontSize:11,margin:"0 0 12px",fontStyle:"italic"}}>Aún no hay sesiones. Crea la primera con "+ Sesión por bloques".</p>; return <div style={{marginBottom:12}}>
      ...
      {sess.slice(0,bsLimit).map(s=>(<Card key={s.id} style={{marginBottom:10,background:C.surface,border:`1px solid ${C.border}`}}>
        ...
        {(s.bloques||[]).map((b,i)=>(<div key={i} style={{marginBottom:6,paddingLeft:8,borderLeft:`2px solid ${b.tipo==="wod"?color:C.border}`}}>
          ...
          {(b.resultado||b.rpe||(b.tipo==="fuerza"&&b.kg))&&<p style={{color:C.text,fontSize:11,margin:"2px 0 0",fontWeight:600}}>{b.tipo==="fuerza"&&b.kg?(()=>{ const prev=bsLastFuerza(b.titulo,s.id); const up=prev&&safeNum(b.kg,0)>prev.kg; return <>{b.kg}kg{up&&<span style={{color:C.green}}> ▲</span>}</>; })():b.resultado}{b.resultado&&b.rpe?" · ":""}{b.rpe?`RPE ${b.rpe}`:""}{b.tipo==="wod"&&(b.rx==="rx"||b.rx==="scaled")?<span style={{color:C.textMuted,fontWeight:500}}>{` · ${b.rx==="rx"?"RX":"Scaled"}`}</span>:null}{b.tipo==="wod"&&b.resultado?(()=>{ const pv=bsLastWod(b.titulo,s.id); if(!pv) return null;
            const cv=b.scoreType==="fortime"?b.secs:b.scoreType==="amrap"?((b.wRounds||0)*1000+(b.wReps||0)):null;
            const cmp=bsWodCmp(b.scoreType||"",cv,pv);
            return <>{cmp===1&&<span style={{color:C.green,fontWeight:700}}>{" ▲"}</span>}{cmp===-1&&<span style={{color:C.orange,fontWeight:700}}>{" ▼"}</span>}<span style={{color:C.textMuted,fontWeight:500}}>{` · ant: ${pv.resultado}`}</span></>; })():null}</p>}
        </div>))}
...
function GymPanel({initPlanoKey=null,autoStart=false,onSessionStarted}){
  ...
  function handleSaveEj(ejId,descanso,numSeries){
    ...
    if(lt==="bodyweight_reps"){
      sets=arr.map((o,i)=>{ const r=parseInt(o&&o.reps); const aw=pkg(o&&o.addedWeight); const rp=(o&&o.rpe!==undefined)?o.rpe:(rpeMode&&_lastSets[i]&&_lastSets[i].rpe!=null?_lastSets[i].rpe:undefined); return {kg:0,reps:isNaN(r)?null:r,addedWeight:isNaN(aw)?0:(aw>0?aw:0),type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.reps!=null&&s.reps>0);
      if(sets.length<numSeries) return; vals=sets.map(s=>s.reps);
    } else if(lt==="duration"){
      sets=arr.map((o,i)=>{ const sec=parseSecs(o&&o.secStr); const rp=(o&&o.rpe!==undefined)?o.rpe:(rpeMode&&_lastSets[i]&&_lastSets[i].rpe!=null?_lastSets[i].rpe:undefined); return {kg:0,seconds:sec,type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.seconds>0);
      if(sets.length<numSeries) return; vals=sets.map(s=>s.seconds);
    } else if(lt==="cardio"){
      sets=arr.map((o,i)=>{ const sec=parseSecs(o&&o.secStr); const dist=pkg(o&&o.dist); const rp=(o&&o.rpe!==undefined)?o.rpe:(rpeMode&&_lastSets[i]&&_lastSets[i].rpe!=null?_lastSets[i].rpe:undefined); return {kg:0,seconds:sec,distance:isNaN(dist)?0:(dist>0?dist:0),type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.seconds>0||s.distance>0);
      if(sets.length<numSeries) return; vals=sets.map(s=>s.distance||s.seconds);
    } else {
      sets=arr.map((o,i)=>{ const kg=pkg(o&&o.kg); const r=parseInt(o&&o.reps); const rp=(o&&o.rpe!==undefined)?o.rpe:(rpeMode&&_lastSets[i]&&_lastSets[i].rpe!=null?_lastSets[i].rpe:undefined); return {kg:isNaN(kg)?0:kg,reps:isNaN(r)?null:r,type:(o&&o.type)||"normal",...(rp!=null&&rp!==""?{rpe:Number(rp)}:{})}; }).filter(s=>s.kg>0);
  ...
  return <div>
    ...
    <div style={{marginBottom:12}}>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:8}}>
        {(()=>{ const chip=(on)=>({background:on?C.card:C.surface,color:on?C.text:C.textSub,border:`1px solid ${on?C.text:C.border}`,borderRadius:8,padding:"5px 11px",cursor:"pointer",fontSize:12,fontWeight:on?600:400}); return <>
          ...
          {programs.map(pr=>(<button key={pr.id} onClick={()=>{ setProgSel(pr.id); const f=safeArr(pr.planos).filter(k=>planos[k]); if(f.length&&!f.includes(planoSel)){ setPlanoSel(f[0]); setSessionSaved({}); setSeriesInputs({}); setSessionRPE(null); setSessionNota(""); } }} style={chip(progSel===pr.id)}>{pr.nombre||"Programa"}</button>))}
    ...
    {(()=>{
      ...
      const last8=keys.slice(-8).map(k=>({fecha:k.slice(5),val:wk[k].sets}));
      ...
      return <Card style={{marginBottom:12}}>
        ...
        {gpOpen&&<div style={{marginTop:10}}>
          ...
          <BarChart data={last8} color={plano.color} unit=" series"/>
